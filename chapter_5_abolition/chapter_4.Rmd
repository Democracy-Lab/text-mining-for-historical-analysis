---
title: "Chapter 4: Distinctive Language in the 1833 Debates About the Abolition of Slavery and 1832 Reform Bill"
output: 
  html_document
...
One of the most important tasks in history is the study of events—moments when something changed. Historians ask not only what happened, but why those moments mattered and how people understood change as it unfolded. In this and future chapters, we adopt a historian's perspective by asking what makes certain moments historically significant and how evidence allows us to interpret them.

This chapter introduces a research strategy. Up to this point, each analytical technique has been practiced separately: counting words, identifying prolific speakers, examining preferred vocabulary, and analyzing collocates to understand how meaning forms around key terms. Now, these skills come together into a single coherent research task centered on understanding a historical event. Events punctuate time, marking moments when attention intensifies and language shifts in response to political or social change. Rather than studying discourse in isolation, the goal is to identify an event, find the speakers most active around it, examine the themes and collocates they used, compare how different individuals framed the moment, and interpret what changed and why it mattered. In earlier exercises, speakers, word use, and collocates were analyzed independently; here, those same methods are combined into a unified workflow that moves from computational analysis toward historical interpretation.

That strategy is not just about measuring words or applying computational tools; it involves deciding which words are worth following and allowing time for curiosity about what interpretation makes possible. Learning to text mine is not entirely about code—it is about judgment. How do we decide which words are interesting? How do we recognize when a pattern is meaningful? How do we move from raw counts to historical argument?

To see how these questions emerge in practice, we turn to a historical example whose meaning has shifted dramatically over time. The British Abolition of Slavery in 1833, alongside the abolition of the slave trade by British naval vessels in 1806, were once celebrated as the sign that Britain was a nation committed to moral progress around the world. That claim has been significantly modified by historical research in recent years, as researchers have underscored that the abolition of slavery failed to end many forms of slavery; it wooed slave owners through instituting a prolonged form of slavery called "apprenticeships." In India and the rest of Asia, slavery meanwhile persisted unreformed. 

Studying the interplay between white male politicians as they debated the fate of millions of enslaved persons from the West Indies to South Africa is in many ways an out-of-date exercise. Today, we recognize the degree to which enslaved people across British empire struggled to liberate themselves. A persistent tradition of uprisings and riots marked their ongoing resistance. The life of ideas and leaders — even women leaders like Queen Nannie — are important to understanding the British Empire.  

We will find only an indirect trace of how the freedom struggles of the Caribbean changed laws and ideas in Britain. The personages and uprisings that rocked the West Indies were rarely named in Westminster. The direct records of those events are held in pamphlets and documents in the Caribbean itself, slowly being digitalized and published through other means.[^1]: One of the authors of this book has been working with translators who travel to the National Archives of Cuba to photograph notarial records that contain references to slavery. Buongiorno then processes the images using a large language model (LLM) to perform optical character recognition (OCR), after which the transcriptions are validated for accuracy.
...
"Background knowledge" in a subject field refers to the kind of knowledge that one acquires by reading the major textbooks and secondary monographs from the field of History that cover the historical time period in question. Textbooks and secondary monographs provide extensive knowledge about which sources have been consulted, what scholars have debated, whether the actions of kings or parliaments or working-class people mattered at this period of time and why.  
...
In 'hybrid' work, where computer scientists and humanists work on the same team to produce new knowledge, collaborating over multiple iterations and investigations, there can be a division of labor around the research. In general, a high caliber of work can only be accomplished by teams where at least one valued member is as serious about history as they are about algorithmic study.
...
In a previous chapter, we looked into the top speakers in parliament in a given year. In this exercise, we want to begin by finding just those speakers who spoke the most in debates about slavery. We can use `str_detect()` to look for the word "slavery" in the titles of debate. Then we count the top speakers.

The code below is otherwise very similar to the code in the previous chapters. One of the great pleasures of coding is the facility with which a few skills can quickly become powerful in the hands of an analyst who has many questions.
...
head(top_slavery_speakers)
```

We now have a dataset that shows the total number of words by each speaker in the subset of debates about slavery from the 1830s. We can now find word counts per speaker by year. 
...
words_per_speaker_per_year_slavery_1830 <- slavery_words_1830 %>% # create a new dataset
  ...
  summarize(words_per_speaker_per_year = n()) %>% # count words and store in new column
  arrange(desc(words_per_speaker_per_year)) # sort in descending order of word count
...
words_per_speaker_per_year_slavery_1830 <- slavery_words_1830 %>% # create a new dataset
  group_by(speaker, year) %>% # group by speaker and year
  summarize(words_per_speaker_per_year = n()) %>% # count words per speaker per year
  arrange(desc(words_per_speaker_per_year)) # sort from highest to lowest word count

# find the words per speaker per year for just the top speakers
top_words_per_speaker_per_year_slavery_1830 <- words_per_speaker_per_year_slavery_1830 %>%
  filter(speaker %in% unique(top_slavery_speakers$speaker)) # retain only relevant rows
```

```{r}
head(top_words_per_speaker_per_year_slavery_1830)
```

```{r}
tail(top_words_per_speaker_per_year_slavery_1830)
```
We can now visualize our data to view who spoke the most about slavery, and in what years. We can also introduce another library, `ggrepel`, which gives us tools for enhancing the readability of our plot by making sure that labels do not overlap.
...
ggplot(top_words_per_speaker_per_year_slavery_1830, # make a graph
       aes(x = year, # x axis = year
           y = words_per_speaker_per_year, # y axis = word count
  ...
  scale_x_continuous(breaks = sort(unique(top_words_per_speaker_per_year_slavery_1830$year))) # create a whole number x-axis
```
...
In the following code, we provide a list of speakers and use the `%in%` operator to filter for debates that contain any of these speakers. 

```{r, message = F}
# make a list of the names of the top speakers
most_active_speakers = c("Mr. Fowell Buxton", "Mr. Buckingham", "Mr. Stanley", 
             "The Duke of Wellington", "Sir Robert Peel")

# search for the words said by the speakers in most_active_speakers 
top_slavery_speakers_1833 <- slavery_debates_1833 %>%
  filter(speaker %in% most_active_speakers) # keep rows containing a speaker's name

head(top_slavery_speakers_1833)
```

Next, let's break the text into words and clean it up, removing numbers and stop words. We are lemmatizing each word to its base form so that we can count singular and plural of the same word as one.
...
filtered_words_per_top_slavery_speaker_1833 <- top_slavery_speakers_1833 %>%
  ...
  filter(!str_detect(word,"[0-9]+")) %>% # remove all numbers 
...
Next, we will count how many times each word appears for each speaker in the vocabulary of each abolitionist speaker.

```{r, message = F}
abolition_speaker_wordcounts <- filtered_words_per_top_slavery_speaker_1833 %>%
...
top_words_per_abolition_speaker <- abolition_speaker_wordcounts %>%
  slice(1:15)  # find the 15 words that each speaker says the most frequently 
  
head(top_words_per_abolition_speaker)
```

```{r}
tail(top_words_per_abolition_speaker)
...
We already have enough material to hypothesize about some major differences that marked the speakers out in their construction of the problems of freedom and slavery. Buckingham and Buxton, representing the abolition lobby, use a now offensive term for indicating race. The word they use began with a reference to a color, black, indicated by the Latin _nigrum_. We might become curious about how universally this term was used against other comparative terms for race, by whom, and what other words kept company with those terms. Buckingham also uses words for abstract values such as "enjoyment" and “life,” where Buxton is more willing to talk about facts on the ground in "Jamaica," for instance the "insurrection."
...
tfidf_per_top_slavery_speaker_1833 <- abolition_speaker_wordcounts %>% 
  bind_tf_idf(word, speaker, n) %>% 
  arrange(desc(tf_idf))

# view the 50 first rows
head(tfidf_per_top_slavery_speaker_1833, 50)
```
...
head(most_distinctive_words_per_abolition_speaker)
```

Next, we will create a series of small tables displaying each speaker, their most distinctive words, and the `tf_idf` score of that word-speaker combination. 
...
ggplot(data = most_distinctive_words_per_abolition_speaker,  
       aes(x = word, # words put on x-axis (flipped later for readability)
           y = n, # counts of each word put on y-axis
           fill = factor(tf_idf))) + # fill with tf-idf score (converted to a factor for bar shading)
  geom_col() + # draw the bar chart
  facet_wrap(~speaker, scales = "free") + # creates separate mini-plots by speaker
  ...
  scale_y_continuous(breaks = pretty_breaks()) + # choose clean tick marks for the y-axis
  labs(title = "Speakers' Most Distinctive Words",           # labs(): titles, subtitles, captions, axis labels
...
ggplot(data = most_distinctive_words_per_abolition_speaker2, 
       aes(x = word, # words placed on the x-axis (flipped later for readability)
           y = n, # counts of each word placed on the y-axis
           fill = factor(tf_idf))) + # fill shading reflects the tf-idf score
  geom_col() + # draw the bar chart
  scale_fill_grey() + # use grayscale shading instead of color
  facet_wrap(~speaker, scales = "free") + # create separate panels for each speaker
  coord_flip() + # flip the axes to make long word labels readable
  scale_x_reordered() + # preserve word ordering within each facet
  guides(fill = "none") + # hide the fill legend
  scale_y_continuous(breaks = pretty_breaks()) + # choose clean tick marks for the y-axis
  labs(title = "Speakers' Most Distinctive Words with N > 5", # plot titles and labels
...
head(mr_buckingham_slavery_debates)
```
...
ggplot(data = mr_buckingham_slavery_debates,
  aes(x = reorder(word, n), # order words by frequency
      y = n, # counts of how often each word appears
      fill = factor(tf_idf))) + # shading reflects the tf-idf score
  geom_col() + # draw the bar chart
  scale_fill_grey() + # use grayscale shading instead of color
  scale_y_continuous(breaks = pretty_breaks()) + # choose clean tick marks for the y-axis
...
ggplot(data = mr_buckingham_slavery_debates2,
       aes(x = reorder(word, n), # the x axis will show words ordered by their count 
           y = n, # the y axis will show n
           fill = factor(tf_idf))) + # shading reflects the tf-idf score
  geom_col() + # create a bar graph 
  scale_fill_grey() + # use grayscale shading instead of color
  scale_y_continuous(breaks = scales::pretty_breaks()) + # choose clean tick marks for the y-axis
...
mr_buckingham_most_distinctive_word <- tfidf_per_top_slavery_speaker_1833 %>%
  filter(speaker == "Mr. Buckingham") %>%
  arrange(desc(tf_idf)) %>% 
  slice(1) # <------- the only line that changed; retains the first line only
...
buckingham_on_wages <- buckingham_sentences %>%
  filter(str_detect(text, "wages"))

buckingham_on_wages %>%
  mutate("Mr. Buckingham's mentions of wages in the slavery debates" = text) %>% # rename the text column 
  select("Mr. Buckingham's mentions of wages in the slavery debates") %>% # only the one column 
  kable()
```

Here they are printed without `kable` styling, straight to the console. 
...
This version is much harder to read.  

Reading is the essential reason for any choice of formatting. We may expand the number of lines or characters we display by adding an argument to head() (as in head(50) for fifty lines) or changing the second argument of str_wrap(text, 80)) to show a higher number of characters. The analyst's responsibilty is to use continuously adjust the number of rows of data and characters in order to skim the actual text in question in such a way that promotes their learning -- and the learning of any readers whom they intend to educate about their argument. 

When we read, we produce understanding.  In the lines above, we learn from reading that  Buckingham is using the word "wages" in concert with the concepts of "freedom," "labour," and "property" to recapitulate a Lockean argument about how property depends on the work of men to enclose land, and how the duty fo the state is to uphold contracts that will protect exchanges built upon this system, upon which capitalism rests.  A string of sentences follow about the specificities of wages. But the details are hard to interpret, in part because we are missing important context. 
...
buck_sentence_ids <- buckingham_sentences %>% 
  select(sentence_id) 

# find the ids of Buckingham's speeches:
buck_speech_ids <- buck_sentence_ids %>%
  left_join(file_metadata_1830) %>%  # find the file metadata, including speech_id, for each sentence
  select(speech_id) # retain just the speech_id
  
# find all the sentences that correspond with those speeches
buck_speeches <- buck_speech_ids %>%
  left_join(file_metadata_1830) %>% # call up all the sentence_id's that correspond to the speech id's %>%
  select(sentence_id) %>% # we need only the sentence_id column 
  left_join(hansard_1830, relationship = "many-to-many") # call up the sentences that correspond to those sentence_ids
...
```{r, message = F}
# Find the speeches about slavery where Mr. Buckingham mentions 'enjoyment'
buckingham_sentences_enjoyment <- slavery_debates_1830 %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham",
         str_detect(text, "enjoyment"))

# make a table
buckingham_sentences_enjoyment %>%
  mutate("Mr. Buckingham's mentions of enjoyment" = text) %>% # rename the text column 
  select("Mr. Buckingham's mentions of enjoyment") %>% # only the one column 
  kable()
```
...
```{r, message = F}
# filter Mr. Buckingham's contributions to the slavery debates for discussions of love.
buckingham_sentences_love <- slavery_debates_1830 %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham",
         str_detect(text, "love"))

# make a table
buckingham_sentences_love %>%
  mutate(text = str_wrap(text, 80)) %>% # just the first few lines
  mutate("Mr. Buckingham's mentions of love" = text) %>% # rename the text column 
  select("Mr. Buckingham's mentions of love") %>% # only the one column 
  kable()
```
...
```{r, message = F}
# fine just those sentences where Mr. Buckingham says 'universal'
buckingham_sentences_universal <- slavery_debates_1830 %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham",
         str_detect(text, "universal"))

# make a table
buckingham_sentences_universal %>% head() %>% # just the first few rows
  mutate(text = str_wrap(text, 80)) %>% # just the first few lines
  mutate("Mr. Buckingham's mentions of the universal" = text) %>% # rename the text column 
  select("Mr. Buckingham's mentions of the universal") %>% # only the one column 
  kable()
```
...
```{r, message = F}
buckingham_sentences_desire <- slavery_debates_1830 %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham",
         str_detect(text, "desire"))

buckingham_sentences_desire %>%
  head() %>% # just the first few rows
  mutate(text = str_wrap(text, 80)) %>% # just the first few lines
  mutate("Mr. Buckingham's mentions of desire" = text) %>% # rename the text column 
  select("Mr. Buckingham's mentions of desire") %>% # only the one column 
  kable()
```
...
missing_context %>%
  head() %>% # just the first few rows of data
  mutate(text = str_wrap(text, 80)) %>% # just the first few characters
  select(text) %>% # just the text column 
  kable()
```
...
peel_sentences <- slavery_1830 %>% # start with the speeches about slavery 
  left_join(speaker_metadata_1830) %>% # add information about the speakers of the speeches
  filter(speaker == "Sir Robert Peel", # find just the speeches where Peel is the speaker and he mentions the assembly 
         str_detect(text, "Assembl|assembl")) # look for uppercase OR lowercase (| = "or")

# make a table
peel_sentences %>%
  head() %>% # just the first few rows
  mutate(text = str_wrap(text, 80)) %>% # wrap 
  select(text) %>% #just the text colum 
  rename("Mr. Peel's sentences about slavery where he mentions the word 'assembly'" = text) %>% # change the name of the column
  kable() 
```
...
# make a dataset of just sentence_ids and the text column 
hansard_trim <- hansard_1830 %>%
  select(sentence_id, text)

# make a dataset for matching sentence_id and speaker
speaker_meta_trim <- speaker_metadata_1830 %>%
  select(sentence_id, speaker)

# make a dataset for matching sentence_id and speech_id
file_meta_trim <- file_metadata_1830 %>%
  select(sentence_id, speech_id)

# make a dataset of just the sentence ids and speech dates
debate_meta_trim <- debate_metadata_1830 %>%
  select(sentence_id, speechdate)

# find the full text of the speeches where the original sentences originated
peel_speeches <- peel_base %>% # start with the sentences 
  left_join(hansard_trim,  by = "sentence_id") %>% # find their sentence_id numbers
  left_join(speaker_meta_trim,  by = "sentence_id") %>% # find the corresponding speakers
  left_join(file_meta_trim, by = "sentence_id") %>% # find the corresponding speech id's 
  left_join(debate_meta_trim, by = "sentence_id") %>% # find the rest of the sentences for those speeches
  group_by(speech_id, speaker, speechdate) %>% # prepare to aggregate data by speech, retaining information about each speech's speaker and date
  summarise(speech = paste(text, collapse = " "), .groups = "drop") %>% # glue together the sentences from each speech 
  mutate(speech = paste0(speechdate, ": ", toupper(speaker), ": ", speech)) %>% # glue together the date, speaker's name, and full speech text 
  select(speech) # retain just one column 

# make a table
peel_speeches %>%
  mutate(speech = str_trunc(speech, 520)) %>%  #
  kable(caption = "Three Speeches by Peel about Slavery Invoking the Word 'Wages'")
```
...
He raised questions about whether slaves were "fit for freedom." The crowning point in Peel's argument was a lesson about history. Peel shocked his listeners by alluding to the liberation of the slaves in the French colony of St. Domingo in 1794, which had been followed riots which Peel described as "atrocities," which resulted in the leveling of inhabitations and the enslavement of whites by black pirates. 

In his account, Peel argued that the violence was levied against white bodies by black ones, and that the same specter of black violence awaited Englishmen in the West Indies if they moved towards liberation too quickly.” Replace the entire paragraph with this: “Peel questioned whether emancipation should be immediate, arguing that freedom required preparation and social stability. The culminating moment of his speech invoked the example of Saint-Domingue (Haiti), where the upheavals following emancipation during the French Revolution had, in British memory, been associated with violence and devastation. Peel described the events there in terms of disorder and “atrocities,” suggesting that sudden liberation without safeguards could produce social breakdown. His reference functioned less as a detailed historical account than as a cautionary analogy: a warning that precipitous emancipation in the British West Indies might generate instability for both formerly enslaved people and colonial society more broadly. In this framing, the lesson of Saint-Domingue was not emancipation itself, but the dangers of unmanaged transition.
...
5. Try using AI to summarize what the speakers said, how they converge and how they differ. Cut and paste the slavery_speakers (check the name!) dataset into an AI prompt. What are the results? Did the AI highlight any of the most distinctive words, or did it move straight to summary? If an AI performs a summary accurately but doesn’t highlight the speakers’ most distinctive words, what would you be missing?
...
7. In our own readings, we shift attention from Westminster debates to enslaved peoples’ resistance in order to reconsider what counts as a “cause” of abolition. In the debates, test whether enslaved people appear as political actors or passive subjects. Do MPs talk about enslaved people differently than they talk about planters or parliament? Which actors appear most often? Are enslaved people described as agents or objects?
...
actor_terms <- c(
  "slave", "slaves", "negro", "negroes", "planter", "planters", "assembly", 
  "parliament")

actor_pattern <- str_c("\\b(", str_c(actor_terms, collapse = "|"), ")\\b")

actor_mentions <- slavery_debates_1833 %>%
  unnest_tokens(word, text) %>%
  filter(word %in% actor_terms) %>%
...
```{r}
actor_sentences <- slavery_debates_1833 %>%
  mutate(
    text_lower = tolower(text),
    actor = str_extract(text_lower, actor_pattern)) %>%
  filter(!is.na(actor)) %>%
  select(sentence_id, speaker, actor, text)

actor_sentences %>%
  mutate(text = str_wrap(text, 90)) %>%
  slice(1:20)

actor_sentences_all_matches <- slavery_debates_1833 %>%
  mutate(text_lower = tolower(text)) %>%
  filter(str_detect(text_lower, actor_pattern)) %>%
  mutate(actor = str_extract_all(text_lower, actor_pattern)) %>%
  unnest(actor) %>%
  distinct(sentence_id, actor, .keep_all = TRUE) %>%
  select(sentence_id, speaker, actor, text)

actor_sentences_all_matches %>%
  head() %>%
