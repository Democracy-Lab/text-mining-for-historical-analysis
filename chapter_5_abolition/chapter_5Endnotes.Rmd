...
A shortcut which Guldi recommends in The Dangerous Art of Text Mining for those who want to achieve results before enrolling in a specialist course on the historical background material is to engage in an ongoing collaboration with a professor, librarian, or graduate student already familiar with the background material, allowing that scholar’s sense of validation and surprise to guide the work with data. In these days, many LLMs can give adequate summaries of the events that pertain to a database. A useful prompt might look like, “Given the Hansard dataset, what are the most important political events of the 1830s that would appear in these debates? Do not hallucinate.” A request for more information might be, “Tell me about the British Abolition of Slavery in 1833. Use published works only. Do not hallucinate. Give links to the citations.” In our experience, at the current level of LLMs, it is worth checking on some of those links to make sure that they are real.^1^
...
Nevertheless, distant reading--a technique where we read historical patterns across large text collections rather than individual documents--allows us to investigate the dynamics of politics in the British parliamentary system that made the official abolition of slavery possible. We can take a snapshot at the speeches of the most active speakers -- Buckingham, Stanley, Wellington, and Peel -- by comparing their most-invoked words in these debates.
...
top_slavery_speakers_1833 %>%
  ...
  mutate(text = str_trunc(text, 120)) %>%  # just the first 120 characters
  kable()
```
...
kable(top_words_per_abolition_speaker) # make a table
```
...
# inspect the data
tfidf_per_top_slavery_speaker_1833 %>% head(50) %>% # just the first few rows
   kable() # make a table
```
...
```{r}
buckingham_on_wages %>% select(text) %>% 
  mutate(text = str_wrap(text, 80))
```

In the plaintext version, we see artifacts of the OCR process like \n, meaning "line break." It's also harder to read.  
