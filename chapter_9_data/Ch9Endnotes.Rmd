...
data_structures %>%
  kable(format = "latex",
        booktabs = TRUE,
        escape = TRUE,   
        caption = "Common R Data Structures in Text Mining") %>%
...
quanteda_tbl %>%
  kable(
    format  = "latex",
    booktabs = TRUE,
    # let kable escape %, _, &, etc. for us
    escape  = TRUE,
    caption = "Quanteda (tokens, dfm, etc.)"
  ) %>%
  kable_styling(
...
tidyverse_tools_tbl <- tibble(`Tool/Package` = tools,
                              Use = use)
...
tidyverse_tools_tbl %>%
  ...
  add_header_above(c("Helpful Tidyverse-Compatible Tools" = ncol(tidyverse_tools_tbl)))
```
...
6. In the dataset you have identified, what counts as messy? Be specific. 