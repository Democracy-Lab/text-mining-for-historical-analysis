...
The ability to engage fragmented infrastructures under any circumstance provides the key to becoming flexible enough to adopt the many archives that may been needed for a grand project.
...
Although vast quantities of cultural and historical materials are now available online through APIs (Application Programming Interfaces, or mechanisms that allow one piece of software to request and receive data from another), the ability to actually retrieve and work with APIs remains unevenly distributed. Accessibility matters because an increasing share of cultural production is "born digital," meaning the digital record is the original form rather than a later digitized copy.^4^
...
Part of the problem is that very few instructional materials teach the practices of working with APIs in a systematic way. Several works exist that outline the history of APIs, conceptualize them as a scholarly artifact, or discuss their significance to digital scholarship.^5^ However, they do not demonstrate how to use an API. 

Most textbooks treat data as if it were already neatly stored on a local machine, while archivists' how-to guides tend to focus on individual platforms one at a time without offering a transferable workflow. A specialized approach may leave analysts adept at navigating a single repository but unprepared to ingest larger, more heterogeneous collections of digital sources at scale. Lessons on accessing one digital archive at a time rarely arms the analyst with the skills they need to feel comfortable ingesting large amounts of data from diverse sources.^6^
...
APIs are a central mechanism through which analysts can access the vast quantities of data available online. API endpoints function not merely as technical tools, but as gateways into contemporary digital archives. Large-scale datasets—ranging from open government records to social media content—are often made accessible through API endpoints, which return data in structured formats such as CSV, or more commonly, JSON and XML.
...
This URL can be accessed programmatically using R, or the analyst can manually inspect the data by copying and pasting it into a web browser’s address bar. When accessed, this URL returns a structured list of recent submissions to the /r/datascience Subreddit in JSON format.^7^ 
...
`https://open-platform.theguardian.com`.^8^ 
...
In the following code, we use two new R packages: `httr` for sending HTTP requests, and `jsonlite` for parsing JSON data into a tidy data frame that mirrors the data structure we have been processing throughout the book.^9^ 
...
The examples above demonstrate basic interactions with APIs that require only a single, fixed URL. However, many APIs--especially those offering more extensive datasets--support more complex queries. APIs require us to construct URLs, which can be a nontrivial task. Challenges stem from constructnig URLs that often include multiple parameters, such as search terms, filters, authentication keys, and pagination controls, all of which must be formatted correctly to retrieve the desired data. After the data has been via the API, it still needs to be parsed into an R-friendly format. 

Numerous R packages have been developed to simply interactions with APIs. These R packages encapsulate the API's functionality within R functions and data structures, abstracting away the more complex tasks such as URL construction and parsing JSON responses. For example, the rOpenGov universe--composed of a community that develops open-source R packages to increase access to government data--offers several packages that simplify working with APIs, including `usdoj` and `oldbailey`. This section demonstrates the use of `usdoj`.^10^

`usdoj` provides more intuitive access to data from the U.S. Department of Justice's API by wrapping complex API interactions into simple R functions. `usdoj` can be installed directly from the rOpenGov universe. To install it, we first need to tell R to check the rOpenGov repository, and then we can install the package using `install.packages("usdoj")` as usual. By default, R installs packages from CRAN (the Comprehensive R Archive Network), but by adding this additional repository, we can access a wider range of packages.
...
From this point, it is easy to analyze words.^11^

```{r, fig.cap="Top 10 words in United States Department of Justice Blog Entries, retrieved on November 24th, 2025."}
library("tidyverse")
library("tidytext")
...
Here we present a list of APIs for scholarly researcher derived from the University of Virginia Library with description provided by the authors of this book.^12^
...
In the following example, only the titles of the news articles and their associated hyperlinks are retrieved from the feed.^13^
...
While a bulk download option might seem straightforward, it is often overlooked because inconsistencies in how repositories are structured or labeled can make bulk download options difficult to locate. 
...
"If you require the complete data (or the API is not suitable for your needs), it is available in XML format only from the University of Sheffield’s data repository (ORDA): http://dx.doi.org/10.15131/shef.data.4775434."^14^
...
* Kaggle, a data science platform perhaps best known for its competitions, also hosts a wide array of community-contributed datasets, ranging from open science research (such as climate or genomics data) to pop culture topics (like movie ratings, video game sales, or lyrics datasets).^15^

## Working With Bulk Download Data

Importantly, while bulk data downloads seem straightforward, they can be difficult to work with in practice. Many open data portals offer complete dataset exports in formats such as CSV or JSON. The primary advantage of accessing a bulk download is the ability to collect substantial volumes of data in a single step. 
...
The `hansardr` package was built with the challenge of loading large data in mind: the full Hansard corpus was broken into smaller partitions to make it possible to load and analyze a large data set without consuming too many resources. But for other datasets, the analyst may need to "chunk" the data manually to make it manageable on a smaller machine.
...
Below is an example of code for reading a large dataset in 1000 row chunks. For the sake of our practice, we will not download a large dataset onto our computers. We are simply offering a code example. 
...
Below is the same process but automated, so the user will read up to 100,000 rows of the data. 
...
It is possible to download a PDF file directly to one's computer and extract its text content for text mining. The following example demonstrates how the process of downloading PDFs can be automated. 

We will practice with an 1893 translation of _Egil's Saga,_ or the _Skalla-Grímssonar_, one of the classic Icelandic sagas, thought to be written in the 13th century, likely by Snorri Sturluson. We will download _Egil's Saga_ from The Icelandic Saga Database, an online resource that has published a large body of medieval Icelandic literature that describes the events that took place among the Norse and Celtic inhabitants of Iceland during the 10th and 11th centuries CE.^16^

Egil's Saga blends historical narrative, legend, and poetry, while focusing on Egill Skallagrímsson, a fierce warrior and gifted poet. The copy of the text that we are using has been previously OCR'd from an original book, rendered as text, cleaned, and output as a machine-readable text with very little noise -- making it an ideal subject on which to practice reading in data from a PDF.

Please note: The code that follows handles downloading and text extraction, but it does not include functionality for deleting the file afterward. Our omission is intentional: to prevent any risk of accidentally deleting important or unrelated files, we have left the task of file deletion to the user’s discretion. If the script is executed, the reader should manually delete the downloaded PDF file from their computer once it is no longer needed.^17^
...
```{r, fig.cap="Top 20 words in Egil's Saga with stop words removed."}
library("tidyverse")
library("tidytext")
...
The metadata helps an analyst keep track of key information about the book. However, because each row in the author, book, and date columns contains the same values, the metadata is redundantly stored in the data frame. 

```{r}
library(knitr)

tidy_egils_saga_with_metadata %>%
  head() %>%
  kable()
```
...
In R, the `rvest` package facilitates web scraping by parsing HTML content into a structured representation, commonly referred to as the HTML tree.^23^ 

An additional precaution is to add code to slow down the web scraping process.  To avoid overloading the server hosting the webpage, requests are typically rate-limited during web scraping. 

## Practicing Web Scraping: Retrieving the Code for _Text Mining for Historical Analysis_

The following example illustrates the initial step in web scrapings: retrieving the HTML content of the GitHub repository page for this book followed by a snippet of the page's HTML. 
...
To extract the content itself, the analyst uses selectors to locate relevant elements in the HTML tree and then retrieve their text. In practice, scraping involves three major steps: downloading the page's HTML, parsing it into a structured document, and applying CSS or XPath queries to extract the desired nodes. For example, using the `rvest` package in R, the following code retrieves all paragraph text from a webpage using the `p` tag as the selector:
...
In the following examples, an apostrophe is included at the beginning of the name "Mr. Speaker" and a hyphen appears before the name "Lord Ashley."
...
Many guides exist for defining and using regular expressions. Therefore, we will not include one here and instead suggest reading CRAN's vignette on regular expressions (https://cran.r-project.org/web/packages/stringr/vignettes/regular-expressions.html) for a comprehensive encyclopedia of regular expressions in R. 

While effective, writing our own regular expression-based approaches to data cleaning can become increasingly intricate and difficult to scale, such as demonstrated by the following code that relies on `stringr` for handling strings.^27^ 
...
GPT-4 returned the following code: 
...
Since a single regular expression is often insufficient to handle all types of messy input, it may be desirable to apply this cleaning approach iteratively, for example, by using a `for` loop that calls the API for a LLM and dynamically generates regular expressions tailored to the specific cleaning needs of the given text. 

Below, we provide an example of what the iterative steps might look like in pseudo-code, or code that resembles real code in order to communicate an idea, but is not actually intended to run. To use similar code, the reader will need to obtain and configure an API key for access to a LLM. 
...
Digital methods are now deeply embedded in many areas of humanities research. As a result, it is increasingly necessary to move beyond a surface-level familiarity with the technical aspects of computational text analysis. We argue it is necessary to approach digital media not merely as "tools," but as complex systems shaped by specific technical features. A conceptual understanding of how data is structured, processed, and interpreted, we suggest, is helpful for engaging more critically with digital sources, and for integrating these methods into the interpretive practices of the humanities.

This section introduces core technical concepts—data format, type, and structure—that shape how textual data is processed and analyzed. In many computational disciplines, an understanding of data type, format, and structure are treated as foundational and are typically addressed early, as they serve as the building blocks to more advanced forms of analysis. Throughout _Text Mining for Historical Analysis_ however, we intentionally set these concepts aside because our approach to text mining in R largely handles data complexities. Most of our work involved character data (e.g., words in a debate) and integer data (e.g., frequency counts of top words), which were stored in data frames compatible with the (primarily) `tidyverse` tools we selected. Our deliberate approach allowed us to move straight into historical analysis without worrying about data complexities. Here, we pause to examine the underlying structures more deliberately, as a deeper understanding of them can inform strategies for different types of analyses. This section thus reframes some technical fluency as a critical component of interpretive practice in computational text analysis. 
...
* PDF (Portable Document Format) – A fixed-layout format intended for visual presentation rather than structured analysis. A fixed layout means that the position of text, images, and other elements is preserved exactly as intended across devices and platforms, replicating the appearance of a printed page. PDFs are therefore ideal for human reading and archival purposes but challenging for computational analysis. Extracting text from PDFs—especially those with poor OCR (Optical Character Recognition) quality—can be error-prone and often requires additional cleaning. Still, they remain a common format for digitized historical texts.
* R data files (`.rds`, `.RData`) – R data files are used to store R objects, such as data frames, in a way that preserves their structure and attributes. These are an efficient means of storage and retrieval within the R environment. When using packages like `hansardr`, the analyst loads `.RData` files. 
...
Throughout this book, we have primarily worked with character and integer data. For other types of historical analyses, however, we may encounter data types or values we have not addressed in detail. Below are some common data types in R: 
...
When observing _The Treachery of Images_, the viewer sees a pipe, despite the instructions emblazoned below the image. If an analyst stares at the image and puzzles for hours, they may still see just a pipe. 
...
Not all values have a data type. Analysts delving deeper into programming may encounter values like `NA` and `NULL` often appear in datasets with missing records. Engaging datasets with `NA` and `NULL` values can be confusing because they analyst cannot treat them the same as data that have a type, like character or integer data. 
...
census <- tibble(name = c("Mary Jenkins", "Samuel Price", "Hannah Lee", "Charles Morton", 
                          "Unnamed Child"),
...
We can now process the dataset and observe what happens when we try to implement functions that expect a string for the value of dataset rows. Here, we use `str_detect()` to identify whether or not a value is equal to the word "domesticated." We return our results in a new column named "is_washerwoman." 
...
`continents_population` is the list type. But each item inside the list also has its own type. To inspect the data type, we can loop through the list and print each item's type like so: 
...
For instance, the `quanteda` software package exports many outputs as S3 objects with custom print methods and slot-like components.  The tm and topicmodels package export their outputs as S4 objects, some of which require knowledge of package-specific functions to extract results.^30^  
...
data_structures <- tibble(Structure = c("Matrix", "List", "Data Frame", "Tibble", "S3 Object", "S4 Object"),
...
data_structures %>%
  kable(caption = "Common R Data Structures in Text Mining") 
```
...
1. Why do scholars sometimes build their own archives instead of relying solely on canonical texts or curated corpora?

2. Explain the differences between data format, data type, and data structure, and discuss how misunderstandings about these distinctions could lead to errors or misinterpretations in computational analysis.

3. When used for historical research, why isn't data cleaning a purely technical task but an interpretive one? 

4. Large language models are exceptionally handy tools, but they come with their own risks. What risks are associated with using large language models directly to clean text?

5. Find a dataset online. What is the source of your dataset? Is it from an API, bulk download, RSS feed, PDF, or scraped webpage? Why did you choose this source? 

If your dataset comes from an API:
- What endpoint are you using?
- What parameters control the size or scope of the data returned?
- Are you encountering rate limits?

If your dataset is a bulk download:
- What format is it in (CSV, JSON, XML, etc.)?
- Is it compressed?
- How large is it?

If you scraped your data:
- What HTML elements did you target?

6. Trace the provenance of your data. Who created it? For what purpose? What decisions were made during collection? What is missing? Write a short paragraph describing how the dataset's origin story might shape the conclusions drawn from it.

7. In the dataset you have identified, what counts as messy? Be specific. 