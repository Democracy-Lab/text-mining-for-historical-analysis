...
A shortcut which Guldi recommends in The Dangerous Art of Text Mining for those who want to achieve results before enrolling in a specialist course on the historical background material is to engage in an ongoing collaboration with a professor, librarian, or graduate student already familiar with the background material, allowing that scholar’s sense of validation and surprise to guide the work with data. In these days, many LLMs can give adequate summaries of the events that pertain to a database. A useful prompt might look like, “Given the Hansard dataset, what are the most important political events of the 1830s that would appear in these debates? Do not hallucinate.” A request for more information might be, “Tell me about the British Abolition of Slavery in 1833. Use published works only. Do not hallucinate. Give links to the citations.” In our experience, at the current level of LLMs, it is worth checking on some of those links to make sure that they are real.
...
top_slavery_speakers_1833 %>%
  ...
  kable(format = "html",  
        booktabs = TRUE) %>% # no vertical lines; horizontal lines only 
  kable_styling(full_width = FALSE, position = "left") %>% # improves spacing
```
...
buckingham_on_wages%>%
  ...
  kable(format = "html") %>% # make a table
  kable_styling() # add horizontal lines
```
...
missing_context %>%
  ...
  kable(format = "html") %>% # make a table
  kable_styling() # add horizontal rows
```
...
peel_speeches %>%
  ...
  kable_styling() # horizontal lines 
```
