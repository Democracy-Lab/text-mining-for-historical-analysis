---
title: "Chapter 2: Counting Words in the 19th-Century British Parliamentary Debates"
output:
  html_document
  #pdf_document:
...
The data processing techniques we foreground will lead to our first visualization, where we transform individual utterances or debates into high-level visualizations depicting the frequencies of keywords and their associations.^3^ To achieve this end, we will introduce three basic techniques for processing text. We will: a) access data for the British parliamentary debates of the House of Lords and House of Commons, 1850-59 using the `hansardr` library, b) filter a relevant decade for keywords of interest, and c) count the words that appear in the same context as our keywords in order to study the associations of the keywords in more detail.^4^  

One common way to study a word's context is through collocate analysis—the identification of words that frequently appear near a given keyword within a specified textual window. These neighboring words, or collocates, provide clues about how a term is used, what associations cluster around it, and how its meaning may shift across time or social settings. For historians, collocates can help reveal implicit assumptions embedded in language: whether a group is discussed in economic, moral, or political terms; whether a concept is framed positively or negatively; or how competing synonyms develop distinct connotations. At the same time, collocates offer only a partial view of meaning. They highlight patterns of association, but they do not by themselves show how words function within sentences or arguments. For that reason, researchers often pair collocate analysis with concordance-style approaches such as Keywords in Context (KWIC), which allow closer inspection of usage in specific passages.

Each of the techniques presented in this chapter have both promise and limits.  As our companion volume, _The Dangerous Art of Text Mining_, makes clear, simple word counts are almost never sufficient on their own to support an in-depth analysis of culture.^5^ Before we count words, we need to decide which words are worth counting. Words are not neutral objects: they are historically conditioned artifacts, shaped by the institutions, conflicts, and assumptions of their time. In nineteenth-century Britain, Parliament was not primarily concerned with tables and chairs; it was actively debating gender, property, citizenship, and empire. Questions such as whether a woman could divorce her husband and retain control over her property, how foreigners and colonial subjects should be classified, or how passports and refugee protections ought to function were not peripheral—they were central to parliamentary business. As a result, the vocabulary of parliamentary debate reflects these struggles. The words we choose to examine in this chapter are therefore guided not only by computational convenience, but by historical judgment: we attend to terms associated with gender, class, nationality, and empire because these concepts structured the problems Parliament was trying to solve. Word counting, in this sense, is not a purely technical exercise; it is an entry point into historically situated questions about power, governance, and social change.
...
The authors developed a data package to go with this book, `hansardr`, to give readers an ability to access the Hansard 19th-Century British Parliamentary Debates within the R environment.^6^ This is a cleaned, analysis-ready corpus of the 19th-century British Parliamentary Debates (1803-1899), also known as "Hansard" in reference to Thomas Curzon Hansard, the publisher.  It identifies debates whose records are missing from UK Parliament's corpus, and it also offers a field for disambiguated speakers. We believe this dataset will enable researchers to analyze the Hansard debates, including speaker discourse, in a way that has not been accessible before. Once installed, the data can be loaded with the `data()` function.
...
We also placed a hashtag (`#`) followed by text; in R, this indicates a comment, meaning that we included text for explanation that is ignored when the code runs. We use comments to describe what the code does, explain why we made certain choices, or leave notes for our future selves. Here, we included a comment noting that this line of code loads the `hansardr` data package into the R session so its data is ready for use. 
...
kable(df, 
      caption = "Overview of the hansardr corpus, showing the primary text files and their associated metadata. Each data set is linked through a common sentence ID.")
```
...
# Select the first five rows of the Hansard dataset
df_head <- hansard_1850 %>% 
  slice_head(n = 5)

# Display the selected rows as a formatted table
kable(df_head, 
...
The `tidyverse` was created to order data in "tidy" tables where it is structured as a data frame made up of columns and rows. In other words, the `tidyverse` is built to work with data like `hansard_1850`.^7^
...
In this chapter we will also use the `tidytext` library, developed by David Robinson and Julia Silge, to work with textual data.^8^ The `tidytext` library contains a variety of tools useful for processing text and treating text as data.  Many sections in this book will therefore begin by loading the two libraries `tidyverse` and `tidytext`.
...
While `unnest_tokens()` automatically converted all tokens to lowercase and removed punctuation for us, there may be situations where we want to perform these steps ourselves—for example, when processing text before tokenization or when applying our own custom cleaning rules. Lowercasing our words ensures they are not treated differently by the computer because of capitalization (e.g., Parliament vs. parliament), and removing punctuation prevents symbols such as commas or periods from being counted as part of a word.

These preprocessing steps help to standardize the corpus, making subsequent analysis—such as counting words—more consistent. FFor example, in `hansard_1850`, sentences often include commas and periods attached to words, and the same word may appear with different capitalization. Both punctuation and case variation can impact our analysis unless punctuation is removed and capitalization is standardized.

```{r, results='asis'}
# Display the first six sentences with their sentence IDs 
hansard_1850 %>%
  slice_head(n = 6) %>%
...
```{r, results='asis'}
# Display the first six cleaned sentences with their sentence IDs
hansard_1850_clean %>%
  slice_head(n = 6) %>%
  select(sentence_id, text) %>%
  kable()
...
While the cleaning performed by `unnest_tokens()` or `str_to_lower` are important, they can also be deceptive. For instance, if we are interested in how many times speakers refer to the Bank of England (capital B, capital E), we do not want all text lowercased because we want to be able to look for instances of "the Bank" with capitalization preserved. 
...
# Display the first six rows of the tokenized Hansard word counts
...
# Display the first six rows of Hansard word counts with stop words removed
...
# Without assigning the output to a variable name it will show automatically 
...
```{r show_hansard_coal}
# Load functions for creating formatted tables
library(knitr)

# Display the first six text entries from the filtered Hansard data
hansard_coal %>%
  head() %>%
  select(text) %>%
  kable()
...
# Tokenize and count words in the "text" column of the hansard_coal dataset
# - unnest_tokens() splits the text into individual lowercase words
# - anti_join() removes common stop words (e.g., "the", "and")
# - str_detect() identifies and removes tokens that contain digits (e.g., years, numbers)
# - count() counts the frequency of each remaining word
# - arrange() sorts the word counts in descending order for easier reading
...
# Identify and count words that appear in text containing the word "corn"
# - str_detect() filters rows where "corn" appears as a whole word
# - unnest_tokens() splits the filtered text into individual lowercase words
# - anti_join() removes common stop words (e.g., "the", "and")
# - str_detect() identifies and removes tokens that contain digits (e.g., years, numbers)
# - count() counts the frequency of each remaining word
# - arrange() sorts the word counts in descending order for easier reading
corn_context <- hansard_1850 %>% # Create a new data frame
  filter(str_detect(text, "\\bcorn\\b")) %>% # Filter rows containing "corn" as a whole word
  unnest_tokens(word, text) %>% # Tokenize the "text" column
  anti_join(stop_words) %>% # Remove stop words
  filter(!str_detect(word, "[:digit:]")) %>% # Remove tokens with digits
  count(word) %>% # Count word frequencies
...
Discussions of coal and corn (the grain trade) both exhibit common references to taxation, duties, and time. But gaining insight about these discussions requires us to look more carefully at what separates the two discourses. "Coal" is bound up with the word "public" to a greater degree than "corn," an intriguing fact. Historian Fredrik Jonsson reminds us that coal early became a sacred calf to members of parliament as the increasing popularity of heating, even by very poor families, made it became anathema for politicians to threaten to tax coal. We see that discussions of coal strongly represent allusions to vessels and mines. That's because coal required a completely separate infrastructure from other forms of commerce. Coal was heavy enough that it had to be shipped on boats on canals. The canals and railroads had to reach to the remote regions where coal was located. We can see that this infrastructural dimension defines the discussion of coal as distinct from that around the grain trade. Grain could be shipped via wagon, rail, or canal. It could be grown anywhere. It therefore required much less by way of a parliamentary discussion of the oddities of sustaining coal. As Jonsson explains, understanding the public and infrastructural dimensions of coal is crucial, because the large-scale collective investments subsidized by parliament explain how fossil fuels became a "ratchet" -- that is, how once they were adopted it was almost impossible to step back from them, even though contemporaries understood many of the problems that a fossil-fuel economy would pose to health.
...
# A left join to keeps all rows from hansard_1850
hansard_1850_with_metadata <- left_join(hansard_1850, debate_metadata_1850)

hansard_1850_with_metadata %>%
  head() %>%
  kable(caption = "Excerpt from the 1850 Hansard corpus showing sentence-level 
...
```{r create_corn_df}
# Convert the corn KWIC results to a data frame
corn_df <- as.data.frame(corn_kwic)

# Display the first 15 rows as a formatted table
corn_df %>%
  head(15) %>%
  kable()
```
...
When is collocate analysis the right method? Researchers of conceptual history frequently use collocates to understand the subtle differences between synonyms.  For instance, historian Ruben Ros has investigated the way that Dutch newspapers in the nineteenth century began to use terms such as "foreign," "overseas," and "strange" in the process of constructing a narrative that increasingly linked the dangers of foreign influence and suspicion of ethnic minorities.^11^  
...
Our research questions might necessitate analytical steps that go beyond the built-in functions provided to us in R packages alone. We may want to create our own functions to customize how words are identified, filtered, or visualized so that the analysis reflects our specific research goals. Or, we might want to create reusable code that keeps us from rewriting the same data processing steps over and over. To demonstrate our point, we are going to write a basic, reusable function for counting and visualizing words. First, however, we want to provide a basic understanding of a function's underlying components, by creating a simple function: 
...
# Display the first six rows of the Hansard dataset with metadata
hansard_1850_with_metadata %>%
  head() %>%
  kable()
```
...
In the following code, we use `hansard_1850[[2]][1:10]` to select the first ten text entries from the second column of hansard_1850. The double brackets, `[[2]]`, select the second column, which contains the text of the Hansard debates. We then use `[1:10]` to select the first ten entries from that column.

```{r subset_hansard_1850}
...
In the following code, we use `hansard_1850[[2]][1]` to select specific text from `hansard_1850`. This time we will view just the first row. 
...
Viewing the dataset this way also displays additional formatting that can make longer passages difficult to read. To view the complete text more clearly, double-click the cell, then copy and paste the text into a word processor. The passage reads as follows:
...
2) Some of the terms for women and men on your list may be slangy and pejorative, even offensive. Given our intention of understanding how parliament — a body of privileged white men — talked about the individuals ruled by parliament, do we need to understand the frequency of those terms? Or conversely should they be filtered out because they are insensitive? What is the best approach for your readers?

3) Use your powers of inspecting data to move from the keyword context back to the individual sentences.  Cut and paste twenty sentences from the context of each keyword into a word processor.  What do you learn from these sentences that you did not learn from counting keywords?  Does your analysis become more persuasive when quantitative graphs are accompanied with readings of particular speeches where the words are examined in their original context?  

4) We have examined 'coal' and 'corn' in the 1850s.  Now, search for the words 'coal' and 'corn' in dataset `hansard_1820`.  Were they being used in the same way?  What changed?

5) In "Tailoring our Word Count Analysis", we created the list of words called `identifiers_for_woman`.  Create another list called `identifiers_for_man` and do the same for discussions of men. Create visualizations for the top terms used in discussions of men and women.

6) Query an AI—such as ChatGPT—for alternative words referring to men and women. Then evaluate the results critically. Do the suggested terms seem appropriate for the historical context you are studying? Do they improve your search results, or do they introduce irrelevant or misleading matches? When you use these additional terms in your corpus search, do you retrieve more relevant examples, or simply more noise? Finally, consider whether some suggested words may be anachronistic or culturally inappropriate for the nineteenth century, which could affect the validity of your analysis.
