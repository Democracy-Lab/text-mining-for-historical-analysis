---
title: "Chapter 6: Modeling the Event"
output:
  pdf_document:
...
top_words %>%
  ...
  row_spec(0, bold = TRUE, background = "black", color = "white") %>%   # header styling
  add_header_above(c(" " = ncol(top_words)))  # keeps table width stable
...
# )
# 
# # Define reform cutoff
# reform_cutoff <- as.Date("1832-06-07")
# 
# hansard <- hansard %>%
#   mutate(
#     speechdate = as.Date(speechdate),
#     period = if_else(speechdate < reform_cutoff, "pre_reform", "post_reform")
#   )
# 
# # Tokenize + clean
# tidy_words <- hansard %>%
...
# 
# # Compute TF-IDF where document = period
# period_tfidf <- tidy_words %>%
#   count(period, word, name = "wordsperperiod") %>%
#   bind_tf_idf(term = word,
#     document = period,
#     n = wordsperperiod
#   )
# 
# # Get top words per period
...
# 
# # Plot
# ggplot(top_words, aes(x = word, y = tf_idf, fill = period)) +
...
#   scale_fill_grey() +
#   labs(
#     title = "Distinctive Words Before and After the Reform Act of 1832",
...
# Use count_tokens() to tokenize and count by period
period_counts <- count_tokens(
  data = hansard,
  group = "period",
  text = "text"
) %>%
  anti_join(stop_words, by = "word") %>%
  filter(!str_detect(word, "^[0-9]+$")) %>%
  rename(wordsperperiod = n)

# Compute TF-IDF where document = period
period_tfidf <- period_counts %>%
  bind_tf_idf(
    term = word,
    document = period,
    n = wordsperperiod
...
The results are interesting. One of the top most ranked prereform word is "quamina," a word that references Quamina Gladstone, the leader of the 1823 slave revolt in Demerara led by an enslaved carpenter born in Ghana and sold into slavery with his mother in 1817. Gladstone was prohibited from taking time off of work to look after his ill wife, Peggy. After Peggy's death, Quamina's son Jack Gladstone organized a peaceful strike of thousands. In the aftermath of the revolt, Quamina was assassinated, his body displayed in chains beside the public road.
...
How different the frequency of a word is across two groups

Not which group it belongs to

In your code, for each word:
...
b
b = count in all other groups (here: post-reform)

Then it computes a statistic based on:

observed counts vs expected counts

👉 This produces a magnitude of difference, not a direction.
...
# 
# # Aggregate to counts
# period_counts <- tidy_words %>%
#   count(period, word, name = "n")
# 
# # Compute log likelihood
# ll_output <- log_likelihood(
#   period_counts,
#   group = "period",
#   group_list = c("pre_reform", "post_reform")
# )
# 
# # Since LL is symmetric here, keep one copy of the score
# ll_scores <- ll_output %>%
#   transmute(
#     word,
#     log_likelihood = pre_reform
#   )
# 
# # Pivot counts wide so each word has counts in both periods
# word_counts_wide <- period_counts %>%
#   pivot_wider(
#     names_from = period,
#     values_from = n,
#     values_fill = 0
#   )
# 
# # Total number of tokens in each period
# period_totals <- period_counts %>%
#   group_by(period) %>%
#   summarise(total_n = sum(n), .groups = "drop")
# 
# pre_total <- period_totals %>%
#   filter(period == "pre_reform") %>%
#   pull(total_n)
# 
# post_total <- period_totals %>%
#   filter(period == "post_reform") %>%
#   pull(total_n)
# 
# # Add relative frequencies and assign each word to the period
# # where it is more characteristic
# distinctive_words <- word_counts_wide %>%
#   left_join(ll_scores, by = "word") %>%
#   mutate(
#     pre_rate = pre_reform / pre_total,
#     post_rate = post_reform / post_total,
#     distinctive_period = case_when(
#       pre_rate > post_rate ~ "pre_reform",
#       post_rate > pre_rate ~ "post_reform",
#       TRUE ~ "tie"
#     )
#   ) %>%
#   filter(distinctive_period != "tie")
# 
# # Top words per period
# top_words <- distinctive_words %>%
#   group_by(distinctive_period) %>%
#   arrange(desc(log_likelihood)) %>%
#   slice_head(n = 15) %>%
#   ungroup() %>%
#   mutate(word = reorder_within(word, log_likelihood, distinctive_period))
# 
# # Plot
# ggplot(top_words, aes(x = word, y = log_likelihood, fill = distinctive_period)) +
#   geom_col(show.legend = FALSE) +
#   facet_wrap(~distinctive_period, scales = "free_y") +
#   coord_flip() +
#   scale_x_reordered() +
#   scale_fill_grey() +
#   labs(
#     title = "Distinctive Words Before and After the Reform Act of 1832",
#     subtitle = "Words are assigned to the period where their relative frequency is higher",
#     x = "",
#     y = "log likelihood"
#   ) +
#   theme_minimal()
...
# Keep one LL score column since the statistic is symmetric
ll_scores <- ll_output %>%
  transmute(
    word,
    log_likelihood = pre_reform
...
# Assign each word to the period where its relative frequency is higher
distinctive_words <- word_counts_wide %>%
  left_join(ll_scores, by = "word") %>%
  mutate(
    pre_rate = pre_reform / pre_total,
...
# --- Prereform TF-IDF by speaker class ---
# Requires these objects created earlier in the chapter:
# - hansard (combined decades)
# - reform_cutoff (as.Date("1832-06-07") or similar)
# - all_classes_unique (speaker -> class)

# If all_classes_unique doesn't exist in your current environment, rebuild it safely:
# all_classes_unique <- all_classes %>% distinct(suggested_speaker, class)

prereform_class_words <- hansard %>%
  mutate(speechdate = as.Date(speechdate)) %>%
  filter(speechdate >= as.Date("1822-06-04"),
         speechdate <  reform_cutoff) %>%          # prereform decade
  filter(!is.na(suggested_speaker), suggested_speaker != "") %>%
  left_join(all_classes_unique, by = "suggested_speaker") %>%  # bring in class labels
  filter(!is.na(class)) %>%                      # drop speakers we can't classify
  unnest_tokens(word, text) %>%
  anti_join(stop_words, by = "word") %>%
  filter(!str_detect(word, "^[0-9]+$"))

prereform_words_w_speaker_classifications_tfidf <- prereform_class_words %>%
  count(class, word, name = "wordsperclass") %>%
  bind_tf_idf(term = word, document = class, n = wordsperclass) %>%
  filter(!is.na(tf_idf), is.finite(tf_idf))
...
prereform_top_words_w_speaker_classifications_tfidf <- prereform_words_w_speaker_classifications_tfidf %>%
  ...
  mutate(
    ...
    word  = reorder_within(word, tf_idf, class)
  )
...
ggplot(prereform_top_words_w_speaker_classifications_tfidf,
  ...
  coord_flip() +
  scale_fill_grey() +            # harmless even if unused
  scale_x_reordered() +
  labs(
...
library(ggplot2)
library(forcats)
# library(textstem)   # uncomment if you want lemmatize_words()

# deduplicate class lookup
all_classes_dedup <- all_classes %>%
  distinct(suggested_speaker, .keep_all = TRUE)

# 1) create postreform words first
postreform_words <- hansard %>%
  mutate(speechdate = as.Date(speechdate)) %>%
  filter(speechdate >= reform_cutoff) %>%
  filter(!is.na(suggested_speaker), suggested_speaker != "") %>%
  unnest_tokens(word, text)
...
postreform_words_cleaned <- postreform_words %>%
  ...
  anti_join(stop_words, by = "word")
  # %>% mutate(word = lemmatize_words(word))   # optional

# 3) join speaker classes
postreform_words_w_speaker_classifications <- postreform_words_cleaned %>%
  left_join(all_classes_dedup %>% select(suggested_speaker, class),
            by = "suggested_speaker") %>%
  filter(!is.na(class))

# 4) count tokens per class and word
...
postreform_words_w_speaker_classifications_tfidf <- postreform_words_w_speaker_classifications_counted %>%
  bind_tf_idf(term = word, document = class, n = wordsperperiod) %>%
...
words_w_speaker_classifications <- all_classes_dedup %>%
