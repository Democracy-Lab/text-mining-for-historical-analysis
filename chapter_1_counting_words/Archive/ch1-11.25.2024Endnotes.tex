...
that co-occur with one-another. As the linguist John Rupert Firth
said, ``You shall know a word by the company it keeps.''^1^ Tracking subtle
...
contested or have shifted over time.^2^ For example, the
...
their associations.^3^ To achieve this end, we
...
the associations of the keywords in more detail.^4^
...
Mining}, makes clear, simple word counts are almost never
sufficient on their own to support an in-depth analysis of culture.^5^
...
19th-Century British Parliamentary Debates within the R environment.^6^ This is a cleaned, analysis-ready corpus of the
...
work with data like \texttt{hansard\_1850}.^7^
...
data.^8^ The \texttt{tidytext} library contains a variety of tools useful
...
grammar, order, or context.^9^ The focus is instead on word
...
known as the Corpus Thomisticum.^10^ As a Jesuit scholar deeply
...
between synonyms. For instance, historian Ruben Ros has
...
dangers of foreign influence and suspicion of ethnic minorities.^11^
...
Britain, typically associating inferiority with racial identity.^12^
