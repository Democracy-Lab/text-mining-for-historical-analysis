...
that co-occur with one-another. As the linguist John Rupert Firth said,
``You shall know a word by the company it keeps.''\textsuperscript{1}
Tracking subtle variations in the context with which different words are
used helps historians and other analysts to understand the cultural
biases that structure our collective thinking. They may help us to
understand how a culture understands men and women differently, how a
culture describes people of different nationalities, the difference
between a concept like ``the foreign'' and its near synonyms like
``outsider'' that nevertheless are used in important and different ways,
or how these concepts are changing over time. In the course of this
book, we will offer a taste of the subtle and refined approaches that
analysts have used to unpack the context of different words.
...
contested or have shifted over time.\textsuperscript{2} For example, the
...
their associations.\textsuperscript{3} To achieve this end, we will
introduce three basic techniques for processing text. We will: a) access
data for the British parliamentary debates of the House of Lords and
House of Commons, 1850-59 using the \texttt{hansardr} library, b) filter
a relevant decade for keywords of interest, and c) count the words that
appear in the same context as our keywords in order to study the
associations of the keywords in more detail.\textsuperscript{4}

One common way to study a word's context is through collocate
analysis---the identification of words that frequently appear near a
given keyword within a specified textual window. These neighboring
words, or collocates, provide clues about how a term is used, what
associations cluster around it, and how its meaning may shift across
time or social settings. For historians, collocates can help reveal
implicit assumptions embedded in language: whether a group is discussed
in economic, moral, or political terms; whether a concept is framed
positively or negatively; or how competing synonyms develop distinct
connotations. At the same time, collocates offer only a partial view of
meaning. They highlight patterns of association, but they do not by
themselves show how words function within sentences or arguments. For
that reason, researchers often pair collocate analysis with
concordance-style approaches such as Keywords in Context (KWIC), which
allow closer inspection of usage in specific passages.

Each of the techniques presented in this chapter have both promise and
limits. As our companion volume, \emph{The Dangerous Art of Text
Mining}, makes clear, simple word counts are almost never sufficient on
their own to support an in-depth analysis of culture.\textsuperscript{5}
Before we count words, we need to decide which words are worth counting.
Words are not neutral objects: they are historically conditioned
artifacts, shaped by the institutions, conflicts, and assumptions of
their time. In nineteenth-century Britain, Parliament was not primarily
concerned with tables and chairs; it was actively debating gender,
property, citizenship, and empire. Questions such as whether a woman
could divorce her husband and retain control over her property, how
foreigners and colonial subjects should be classified, or how passports
and refugee protections ought to function were not peripheral---they
were central to parliamentary business. As a result, the vocabulary of
parliamentary debate reflects these struggles. The words we choose to
examine in this chapter are therefore guided not only by computational
convenience, but by historical judgment: we attend to terms associated
with gender, class, nationality, and empire because these concepts
structured the problems Parliament was trying to solve. Word counting,
in this sense, is not a purely technical exercise; it is an entry point
into historically situated questions about power, governance, and social
change.
...
\subsection{Getting Help on Functions}\label{getting-help-on-functions}

When working in R, you will frequently encounter functions you have not
used before. A simple way to learn what a function does is to open its
documentation. We can read its documentation in RStudio by typing two
question marks followed by the function name:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{??function\_name}
\end{Highlighting}
\end{Shaded}

For example, to read the documentation for the \texttt{library()}
function, run:
...
This opens a page in RStudio's Help pane that shows a description of the
function and examples of how to use it.

Learning to consult documentation early will make it much easier to
follow the examples in this book and to experiment on your own.
...
19th-Century British Parliamentary Debates within the R
environment.\textsuperscript{6} This is a cleaned, analysis-ready corpus
of the 19th-century British Parliamentary Debates (1803-1899), also
known as ``Hansard'' in reference to Thomas Curzon Hansard, the
publisher. It identifies debates whose records are missing from UK
Parliament's corpus, and it also offers a field for disambiguated
speakers. We believe this dataset will enable researchers to analyze the
Hansard debates, including speaker discourse, in a way that has not been
...
We also placed a hashtag (\texttt{\#}) followed by text; in R, this
indicates a comment, meaning that we included text for explanation that
is ignored when the code runs. We use comments to describe what the code
does, explain why we made certain choices, or leave notes for our future
selves. Here, we included a comment noting that this line of code loads
the \texttt{hansardr} data package into the R session so its data is
ready for use.
...
\FunctionTok{kable}\NormalTok{(df\_head, }
      ...
      \AttributeTok{caption =} \StringTok{"Excerpt from the 1850 Hansard corpus showing }
\StringTok{                 sentence{-}level text with associated sentence ID values."}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
...
a software package called \texttt{tidyverse} which is a family of
...
work with data like \texttt{hansard\_1850}.\textsuperscript{7}
...
developed by David Robinson and Julia Silge, to work with textual
data.\textsuperscript{8} The \texttt{tidytext} library contains a
variety of tools useful for processing text and treating text as data.
Many sections in this book will therefore begin by loading the two
libraries \texttt{tidyverse} and \texttt{tidytext}.
...
To count words in Hansard, we will use the \texttt{unnest\_tokens()}
function from \texttt{tidytext}. While there are many ways to count
words over time, one way to do this--that is also very human
readable--is to put our data into a format where each word of the text
is on a separate row. These individual words are called ``tokens.'' The
process of splitting chunks of text into individual units is called
``tokenization.'' While singular words are often called ``tokens,'' a
common term used to instead describe multi-word phrases is ``n-gram''
(such as using the term ``bigrams'' to describe sequential, two-word
phrases).
...
grammar, order, or context.\textsuperscript{9} The focus is instead on
...
successive transformations of a dataset. We will continue to use
operators for data processing throughout the book, and they will begin
to make more sense when we cover more examples.
...
\subsection{Lowercasing and Stripping
Punctuation}\label{lowercasing-and-stripping-punctuation}

While \texttt{unnest\_tokens()} automatically converted all tokens to
lowercase and removed punctuation for us, there may be situations where
we want to perform these steps ourselves---for example, when processing
text before tokenization or when applying our own custom cleaning rules.
Lowercasing our words ensures they are not treated differently by the
computer because of capitalization (e.g., Parliament vs.~parliament),
and removing punctuation prevents symbols such as commas or periods from
being counted as part of a word.

These preprocessing steps help to standardize the corpus, making
subsequent analysis---such as counting words---more consistent. FFor
example, in \texttt{hansard\_1850}, sentences often include commas and
periods attached to words, and the same word may appear with different
capitalization. Both punctuation and case variation can impact our
analysis unless punctuation is removed and capitalization is
standardized.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{hansard\_1850 }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{head}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{text =} \FunctionTok{str\_wrap}\NormalTok{(text, }\DecValTok{80}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# wrap long text for readable table output}
  \FunctionTok{select}\NormalTok{(sentence\_id, text) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{column\_spec}\NormalTok{(}\DecValTok{2}\NormalTok{, }\AttributeTok{width =} \StringTok{"14cm"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
sentence\_id & text\\
\midrule
S3V0108P0\_0 & , which had been prorogued successively from the 1st of August to the 9th of
October, from thence to the 20th of November, thence to the 16th of January, and
from thence to the 31st January, met this day for despatch of business.\\
S3V0108P0\_1 & The Parliament was opened by Commission, the LORDS COMMISSIONERS present
being the LORD CHANCELLOR; the LORD PRESIDENT OF THE COUNCIL (the Marquess of
Lansdowne); the LORD PRIVY SEAL (the Earl of Minto); the LORD CHAMBERLAIN OF THE
HOUSEHOLD (the Marquess of Breadalbane); and the LORD BISHOP OF LONDON.\\
S3V0108P0\_2 & called attention to a great omission of their duty on the part of Ministers,
with respect to the privileges of their Lordships, which might and ought to have
been avoided.\\
S3V0108P0\_3 & At pre-sent there were two vacancies in the representative Peers of Scotland, in
consequence of the deaths of the Earl of Airlie and of Lord Colville.\\
S3V0108P0\_4 & Although the Act of Parliament directed that the proclamation should issue
forthwith for the election of representative Peers to fill up any vacancies
which might occur, by death or otherwise, no such proclamation had yet
taken place in the case of the two vacancies he had just mentioned, and the
consequence was, that for twenty-four days after the meeting of Parliament it
was not possible for any Scotch representative Peers to be elected.\\
\addlinespace
S3V0108P0\_5 & The Peerage of Scotland was not therefore represented in Parliament at present
as it ought to be.\\
\bottomrule
\end{longtable}

Now we can transform the text to lowercase and strip punctuation.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# mutate() modifies variables in a dataframe, or creates new ones}
\CommentTok{\# str\_to\_lower() transforms text to lowercase}
\CommentTok{\# str\_remove\_all() removes all matches of a pattern (here, punctuation) from text}
\NormalTok{hansard\_1850\_clean }\OtherTok{\textless{}{-}}\NormalTok{ hansard\_1850 }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{text =} \FunctionTok{str\_to\_lower}\NormalTok{(text), }\CommentTok{\# lowercase}
         \AttributeTok{text =} \FunctionTok{str\_remove\_all}\NormalTok{(text, }\StringTok{"[[:punct:]]"}\NormalTok{)) }\CommentTok{\# strip punctuation}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{hansard\_1850\_clean }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{head}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{text =} \FunctionTok{str\_wrap}\NormalTok{(text, }\DecValTok{80}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# wrap long text for readable table output}
  \FunctionTok{select}\NormalTok{(sentence\_id, text) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{kable}\NormalTok{(}\AttributeTok{format =} \StringTok{"latex"}\NormalTok{,}
...
october from thence to the 20th of november thence to the 16th of january and
from thence to the 31st january met this day for despatch of business\\
S3V0108P0\_1 & the parliament was opened by commission the lords commissioners present being
the lord chancellor the lord president of the council the marquess of lansdowne
the lord privy seal the earl of minto the lord chamberlain of the household the
marquess of breadalbane and the lord bishop of london\\
S3V0108P0\_2 & called attention to a great omission of their duty on the part of ministers with
respect to the privileges of their lordships which might and ought to have been
avoided\\
S3V0108P0\_3 & at present there were two vacancies in the representative peers of scotland in
consequence of the deaths of the earl of airlie and of lord colville\\
S3V0108P0\_4 & although the act of parliament directed that the proclamation should issue
forthwith for the election of representative peers to fill up any vacancies
which might occur by death or otherwise no such proclamation had yet taken place
in the case of the two vacancies he had just mentioned and the consequence was
that for twentyfour days after the meeting of parliament it was not possible for
any scotch representative peers to be elected\\
\addlinespace
S3V0108P0\_5 & the peerage of scotland was not therefore represented in parliament at present
as it ought to be\\
\bottomrule
...
While the cleaning performed by \texttt{unnest\_tokens()} or
\texttt{str\_to\_lower} are important, they can also be deceptive. For
instance, if we are interested in how many times speakers refer to the
Bank of England (capital B, capital E), we do not want all text
lowercased because we want to be able to look for instances of ``the
Bank'' with capitalization preserved.
...
\FunctionTok{head}\NormalTok{(tokenized\_hansard\_counts)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# load tidytext\textquotesingle{}s dataset of common stop words}
\FunctionTok{data}\NormalTok{(}\StringTok{"stop\_words"}\NormalTok{)}

\CommentTok{\# remove common stop words from the tokenized word counts}
\NormalTok{stopworded\_hansard\_counts }\OtherTok{\textless{}{-}}\NormalTok{ tokenized\_hansard\_counts }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# create a new data frame }
  \FunctionTok{anti\_join}\NormalTok{(stop\_words) }\CommentTok{\# exclude words that appear in the stop\_words dataset}
...
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ top\_coal) }\SpecialCharTok{+}  
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Top Words from Sentences Mentioning Coal"}\NormalTok{, }\CommentTok{\# add title}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/visualize_coal_context-1} \caption{Top words in sentences mentioning coal (Hansard, 1850)}\label{fig:visualize_coal_context}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/visualize_corn_context-1} \caption{Top words in sentences mentioning corn (Hansard, 1850)}\label{fig:visualize_corn_context}
\end{figure}
...
Discussions of coal and corn (the grain trade) both exhibit common
references to taxation, duties, and time. But gaining insight about
these discussions requires us to look more carefully at what separates
the two discourses. ``Coal'' is bound up with the word ``public'' to a
greater degree than ``corn,'' an intriguing fact. Historian Fredrik
Jonsson reminds us that coal early became a sacred calf to members of
parliament as the increasing popularity of heating, even by very poor
families, made it became anathema for politicians to threaten to tax
coal. We see that discussions of coal strongly represent allusions to
vessels and mines. That's because coal required a completely separate
infrastructure from other forms of commerce. Coal was heavy enough that
it had to be shipped on boats on canals. The canals and railroads had to
reach to the remote regions where coal was located. We can see that this
infrastructural dimension defines the discussion of coal as distinct
from that around the grain trade. Grain could be shipped via wagon,
rail, or canal. It could be grown anywhere. It therefore required much
less by way of a parliamentary discussion of the oddities of sustaining
coal. As Jonsson explains, understanding the public and infrastructural
dimensions of coal is crucial, because the large-scale collective
investments subsidized by parliament explain how fossil fuels became a
``ratchet'' -- that is, how once they were adopted it was almost
impossible to step back from them, even though contemporaries understood
many of the problems that a fossil-fuel economy would pose to health.
...
known as the Corpus Thomisticum.\textsuperscript{10} As a Jesuit scholar
deeply immersed in Thomistic philosophy, Busa wanted to analyze the use
and meaning of the Latin word ``in''---a preposition with significant
...
\NormalTok{hansard\_1850\_with\_metadata }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{kable}\NormalTok{(}\AttributeTok{format =} \StringTok{"latex"}\NormalTok{, }
        \AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{,}
        \AttributeTok{caption =} \StringTok{"Excerpt from the 1850 Hansard corpus showing sentence{-}level }
\StringTok{                  text joined with debate{-}level metadata, including speech dates }
\StringTok{                  and debate titles."}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{corn\_df }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(corn\_kwic)}

\CommentTok{\# Wrap long contexts so they don\textquotesingle{}t overflow}
\NormalTok{corn\_df }\OtherTok{\textless{}{-}}\NormalTok{ corn\_df }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{pre =} \FunctionTok{str\_wrap}\NormalTok{(pre, }\AttributeTok{width =} \DecValTok{40}\NormalTok{),}
         \AttributeTok{keyword =} \FunctionTok{str\_wrap}\NormalTok{(keyword, }\AttributeTok{width =} \DecValTok{10}\NormalTok{),}
         \AttributeTok{post =} \FunctionTok{str\_wrap}\NormalTok{(post, }\AttributeTok{width =} \DecValTok{40}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{select}\NormalTok{(}\SpecialCharTok{{-}}\FunctionTok{c}\NormalTok{(docname, from, to, pattern))}

\NormalTok{corn\_df }\SpecialCharTok{\%\textgreater{}\%}
...
between synonyms. For instance, historian Ruben Ros has investigated the
way that Dutch newspapers in the nineteenth century began to use terms
such as ``foreign,'' ``overseas,'' and ``strange'' in the process of
constructing a narrative that increasingly linked the dangers of foreign
influence and suspicion of ethnic minorities.\textsuperscript{11}
...
Our research questions might necessitate analytical steps that go beyond
the built-in functions provided to us in R packages alone. We may want
to create our own functions to customize how words are identified,
filtered, or visualized so that the analysis reflects our specific
research goals. Or, we might want to create reusable code that keeps us
from rewriting the same data processing steps over and over. To
demonstrate our point, we are going to write a basic, reusable function
for counting and visualizing words. First, however, we want to provide a
basic understanding of a function's underlying components, by creating a
simple function:
...
\NormalTok{    filtered\_hansard\_1850 }\OtherTok{\textless{}{-}}\NormalTok{ hansard\_data }\SpecialCharTok{\%\textgreater{}\%}
      \FunctionTok{filter}\NormalTok{(}\FunctionTok{str\_detect}\NormalTok{(text, }\FunctionTok{regex}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{b"}\NormalTok{, keyword, }\StringTok{"}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{b"}\NormalTok{), }
                                    \AttributeTok{ignore\_case =} \ConstantTok{TRUE}\NormalTok{))) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# filter rows}
      \FunctionTok{unnest\_tokens}\NormalTok{(word, text) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# tokenize text}
      \FunctionTok{anti\_join}\NormalTok{(stop\_words) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# remove stop words}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/generate_plot_foreign-1} \caption{Top words in sentences mentioning foreign (Hansard, 1850)}\label{fig:generate_plot_foreign}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/generate_plot_colonial-1} \caption{Top words in sentences mentioning colonial (Hansard, 1850)}\label{fig:generate_plot_colonial}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/generate_plot_savage-1} \caption{Top words in sentences mentioning savage (Hansard, 1850)}\label{fig:generate_plot_savage}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/generate_plot_native-1} \caption{Top words in sentences mentioning native (Hansard, 1850)}\label{fig:generate_plot_native}
\end{figure}
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{ch1_files/figure-latex/generate_plot_barbarian-1} \caption{Top words in sentences mentioning barbarian (Hansard, 1850)}\label{fig:generate_plot_barbarian}
\end{figure}
...
Britain, typically associating inferiority with racial
identity.\textsuperscript{12}
...
\item
  Some of the terms for women and men on your list may be slangy and
  pejorative, even offensive. Given our intention of understanding how
  parliament --- a body of privileged white men --- talked about the
  individuals ruled by parliament, do we need to understand the
  frequency of those terms? Or conversely should they be filtered out
  because they are insensitive? What is the best approach for your
  readers?
\item
...
\item
  In ``Tailoring our Word Count Analysis'', we created the list of words
  called \texttt{identifiers\_for\_woman}. Create another list called
  ...
  women.
\item
  Query an AI---such as ChatGPT---for alternative words referring to men
  and women. Then evaluate the results critically. Do the suggested
  terms seem appropriate for the historical context you are studying? Do
  they improve your search results, or do they introduce irrelevant or
  misleading matches? When you use these additional terms in your corpus
  search, do you retrieve more relevant examples, or simply more noise?
  Finally, consider whether some suggested words may be anachronistic or
  culturally inappropriate for the nineteenth century, which could
  affect the validity of your analysis.
\item
