...
tokenized_words <- combined_books %>%
  unnest_tokens(word, text) %>%
  filter(str_detect(word, "^[a-z]+$"))
```

* `^` - Tells the computer to start matching the word at the beginning of the dataframe cell. This ensures the match begins with the very first character. If the token begins with a number or symbol, it will not be kept in the dataset.
* `[]` - Brackets that mean “match any one of these characters.” They define the set of characters that are allowed in the match.
* `a-z` - Allows any lowercase letter from a to z.
* `+` - A symbol meaning "match one or more of the preceding characters." This means the regex will match any number of alphabetical characters, as long as they start from the beginning of the token.
* `$` - Tells the computer to match to the end of the string. This ensures that the token continues with alphabetical characters throughout the entire match. 


The order of the data-processing steps is very important. Normally, a regex such as this would remove words that contain punctuation, for example, `rabbit!`. If punctuation were still present in the tokens, we would need to modify the regex to allow punctuation at the end of a word. However, we already removed punctuation from words with `unnest_tokens()` so we do not need to adjust the regex to handle punctuation.

```{r}
# Count how often each word appears for each author
author_word_counts <- tokenized_words %>%
  count(writer, word) %>%
...
combined %>%
  kable(format = "latex",
        booktabs = TRUE,
        col.names = c("Beatrix Word", "TF-IDF", 
