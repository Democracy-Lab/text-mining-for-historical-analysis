...
[Jo -- needs better intro]

Many of the techniques used in text mining did not originate in history; rather, they were developed in fields such as the natural sciences, information science, and data science, and later adapted for the analysis of language. This cross-disciplinary borrowing is powerful, but at the same time, it requires careful interpretation. When a method is adapted from another domain, it may not map perfectly onto historical language data. Analysts must therefore remain attentive to what the metric actually measures, as well as to the kinds of questions it can and cannot answer.
...
# and handle the  punctuation, quotation marks, and non-ASCII characters that often appear in texts
...
kable(head(distinctively_carroll, 15),
      format = "latex",
      booktabs = TRUE,
      caption = "Lewis Carroll's Most Distinctive Words when Alice 
                is compared with Peter Rabbit",
...
In the following code, we take additional steps to focus our analysis on the frequent words from each decade while avoiding rarer terms. Our approach will provide us with an overview of both decades. 
...
* Normalizing Word Frequencies: The word counts are converted into probabilities by dividing each count by the total number of words in that decade. Normalization ensures that word usage is compared proportionally, rather than based on absolute counts.
...
```{r}
# Combine the top words from both decades with their frequency tables
# Create a dataset containing the union of all top words
# Join the 1840s word counts
# Join the 1850s word counts
# Replace missing counts with zeros so the two decades can be compared directly
word_dist <- data.frame(word = all_top_words) %>%
  left_join(freq_1840, by = "word") %>%
  left_join(freq_1850, by = "word") %>%
  replace_na(list(count_1840 = 0, count_1850 = 0))
...
# Taking the log of zero isn’t allowed
# Therefore, we replace any zeros with a tiny number
# This keeps the math stable without changing the results in a meaningful way.
...
The following code works by: 
* Computing Partial JSD: The script applies the Partial JSD functions to compute how much each word's probability in 1840 deviates from the midpoint distribution (1840 to 1850) and vice versa (1850 to 1840). The partial JSD step captures asymmetrical shifts in word usage.
...
```{r}
# Compute Partial Jensen-Shannon Divergence (JSD) for each word 
# in both directions:
# - From the 1840 distribution to the 1850 distribution
# - And from 1850 to 1840
# This captures how much each word contributes to the overall 
# divergence between decades, depending on the direction of comparison.
word_dist <- word_dist %>%
  rowwise() %>%
  mutate(Partial_JSD_1840 = partial_jsd_1840_to_1850(p_1840, p_1850),
         Partial_JSD_1850 = partial_jsd_1850_to_1840(p_1840, p_1850)) %>% 
  ungroup()

# Assign a direction for each word:
# - 1840 → 1850" if its contribution is higher in that direction,
# - otherwise "1850 → 1840".
# Signed Partial JSD:
# - Negative = more distinctive of 1840
# - Positive = more distinctive of 1850
word_dist <- word_dist %>%
  mutate(Direction = 
           ifelse(Partial_JSD_1840 > Partial_JSD_1850,  # decide dominant direction
                            "1840 to 1850", 
                            "1850 to 1840"),
...
# Rank words by the their contribution to divergence (i.e., absolute Partial JSD)
