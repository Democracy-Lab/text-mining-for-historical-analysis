...
In this chapter, we examine two concepts used across many scientific fields to identify and make visible important features of data: distinctiveness and distance. Distinctiveness measures how characteristic a word is within a particular context—for example, identifying which terms appear unusually often in one set of texts compared to others. Distance, by contrast, measures the similarity or difference between texts or linguistic patterns. Together, these concepts provide a framework for comparing bodies of text and identifying the features that distinguish, for example, a decade, a speaker, or debate from another.
...
The first step is to select and download the books that will form the corpus for analysis. We will collect our books from Project Gutenberg, a digital library that provides free access to books whose copyrights have expired or that have been released for public use. The books can be easily downloaded with R, and they are stored in "plain text"—a format that contains characters and no special encoding for fonts, images, or layouts making them easy to process. To download the books we can locate their URLs and pass them to the `readLines()` function.
...
We can now see the books in the Global Environment panel. However, they are not in a tabular format, so we cannot simply click on them and explore their contents the way we could with the `hansardr` data. 

When working with the `tidyverse`, it is conventional to organize data in a tabular format because it makes the data easier to process. Therefore, we will convert the text into a table before continuing with the analysis.
...
When working with large corpora such as the Hansard debates, our process has been different. Because the collection is extremely large, we often partition the data into smaller subsets--such as decade partitions--and each decade is processed separately. Partitioning the Hansard data is necessary for minimizing memory usage and running the data on a personal computer. However, it also means that the same data processing steps must often be repeated for each dataset. 

In our example, the dataset is small enough that we can combine the books into a single dataset using `bind_rows()`, which will later make it easy to apply methods such as TF-IDF.  
...
At this stage, researchers can also inspect the data and gather preliminary information about the dataset. Looking at the data before running a statistical method can help confirm whether the research question is on the right track. For example, a researcher might selectively look for words already known to be characteristic of each author and make observations based on knowledge of the dataset or the texts themselves. This process can help identify words that appear especially characteristic or unique within the texts. 

However, this type of inspection is limited because the process depends on prior knowledge of which words to examine. This type of inspection can also introduce confirmation bias—that is, the tendency to notice evidence that supports existing expectations while overlooking evidence that does not. Additionally, this type of inspection does not allow for the systematic discovery of all distinctive words in the corpus. Methods such as tf–idf are useful because they identify which words are most distinctive across the entire dataset at scale.

The following code creates a table that allows researchers to manually inspect how often our selected words appear for each author. The results reveal clear differences in how frequently each author uses the selected words, highlighting patterns that align with expectations about each author's style and vocabulary.
...
Distinctiveness measures score the relationships between words and documents across two or more sets of texts. Distinctiveness measures can identify words that are characteristic of a specific author, document, year, or group. Take, for example, Lewis Carroll, a renowned British author of children's literature. Carroll, who had a background in mathematics and logic, was famously inventive with language, especially in his use of nonsense and portmanteau words. He had a strong preference for whimsical and linguistically playful terms—words that stretch meaning or defy conventional usage—such as "slithy," "chortle," and "galumph," -- especially visible in his poem _Jabberwocky,_ which enriches the dreamlike world of _Through the Looking Glass_.^2^
...
We will explain the code in more detail later, but for now, focus on the tf-idf scores for each word-author pair. A higher tf-idf score indicates that a word is more distinctive for a particular author, while a lower tf-idf score indicates that a word is less distinctive. In this table, values close to 0 (including 0.000000) indicate words that are either common across both authors or do not stand out for one author. The higher values—such as rabbit for Beatrix Potter or slithy for Lewis Carroll—are still small in absolute terms, but are relatively higher than the others in the table and therefore more distinctive.
...
# View selected words with more readable tf-idf values
...
Let's interpret the results: Although "she" appears much more often in Lewis Carroll than in Beatrix Potter, tf-idf still assigns it a score of zero because the word appears in both authors’ texts. In a corpus with only two documents, any word shared by both documents receives an inverse document frequency of zero, which makes the overall tf-idf score zero. Thus, tf-idf measures distinctiveness across documents, not simply differences in raw counts. This is also a good example of a limitation of tf-idf in a very small corpus. With only two documents, shared words get flattened to zero even when the counts differ a lot.


Rabbit only appears in Beatrix Potter 

In a larger corpus, tf-idf does not reduce all shared words to zero. Instead, it assigns lower scores to words that appear in many documents and higher scores to words that appear in fewer documents. This allows tf-idf to capture degrees of distinctiveness, rather than treating distinctiveness as an all-or-nothing property.
