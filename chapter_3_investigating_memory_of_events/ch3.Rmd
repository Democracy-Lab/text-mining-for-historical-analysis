...
As with the previous chapter, this chapter will emphasize the skills of moving from analyzing text as data to performing a close reading of historical speeches in context. It models the iterative process of working with a controlled vocabulary and reading words in context with the idea of demonstrating a method of inquiry about how and why events happen. The approach we model provides analysts with the tools of deciding for themselves how the patterns identified throughout text mining relate back to their primary sources, that is, the full Hansard debates. As we had previously discussed, our analytic process is iterative, emphasizing that researchers transition from data processing to counting words to applying metrics on those word counts to visualizing trends, and then repeating this deductive process. Throughout the process, it may prove valuable to inspect and re-evaluate the intermediary steps that were taken to arrive at any conclusions about the corpus. This cyclical process may lead to additional historical questions and associations, which may mean more measuring, more visualization, and more guided reading in pursuit of a historical analysis.  
...
```{r}
# Display the sentences as a formatted table
df_head %>%
  kable(caption = "Sample sentences from 1860s Hansard, including each sentence's 
                   unique identifier and the corresponding text as extracted from 
...
# Display the debate-level metadata as a formatted table
df_head %>%
  kable(caption = "Debate-level metadata from 1860s Hansard, including each sentence's 
                   identifier, the date of the speech, and the title of the corresponding debate.")
```

`hansardr` includes detailed contextual information for each of the speeches, using the same system of references that we saw in the previous data set -- series, volume, page, and sentence number. In the `debate_metadata` dataset, however, there is no "text" column, but rather a series of columns that illuminate multiple "fields" of each sentence: the speech date (recorded as "speechdate" within the data) for when the speech was given and the official title of the "debate" in which the sentence was spoken. 
...
# Display the speaker metadata as a formatted table
df_head %>% 
kable(caption = "Speaker metadata for sample sentences from the 1860 Hansard corpus, 
                 including the recorded speaker name, the algorithmically suggested speaker 
                 match, and indicators for name ambiguity, fuzzy matching, and ignored cases.")
```
...
# Display the first six rows of the 1860 Hansard debate text and metadata
hansard_1860 %>%
  head() %>%
  mutate(text = str_trunc(text, 120)) %>%  
  kable(caption = "Text excerpts from the 1860 Hansard corpus alongside their associated 
                   metadata, including the sentence identifier, speech date, and debate title.")
```
...
In the example above, both `hansard_1860` and `debate_metadata_1860` share a column called `sentence_id`. The shared `sentence_id` column acts as a common identifier, enabling us to link the speaker and date information from one dataset to the corresponding text of the speech in the other dataset. By using this common identifier, the two datasets can be "joined" together to create a single, unified dataset that we will use for text processing. 
...
While there are several approaches to finding multiword phrases, we will demonstrate finding them by tokenizing the text into bigrams. 
...
# Create a new data frame "hansard_1867" from "hansard_1860" with data for 1867 
...
The following code uses the `unnest_tokens()` function, which we previously used to split text into individual words. This time, we adjust the function to extract pairs of consecutive words instead. To focus on pairs of consecutive words instead, we set the token argument to n-grams, instructing R to create sequences of consecutive words, and the `n` argument to `2`, specifying that each sequence should consist of two words. The two arguments work together to extract bigrams. 
...
# Display the raw bigrams and associated metadata as a formatted table
df_head %>%
  kable(caption = "Raw bigrams extracted from the 1867 Hansard proceedings alongside 
                   their associated metadata.")
```

Note that the bigrams extracted overlap, as if the same sentence had been subdivided several times. The presentation of the bigrams is expected, as bigrams are created by looking at each pair of words in order, one step at a time—so each new bigram shares a word with the one before it.
...
The `filter(!str_detect(bigram, paste0(...)))` line removes rows where the bigram column contains a stop word. The `paste()` function combines the words in `stop_words$word` using the or operator (`|`), while `paste0()` adds word boundaries (`\\b`) around the regular expression so that only complete words are matched. The `str_detect()` function then identifies bigrams containing these stop words, and the `!` reverses the result so that only bigrams without stop words are retained.

`filter(str_detect(bigram, "^[a-z]+ [a-z]+$"))` keeps rows where the bigram column follows a specific pattern. The bigram must contain exactly two lowercase alphabetic words separated by a single space. Bigrams containing numbers, punctuation, or uppercase letters are excluded.

```{r}
# Clean the bigrams in the 1867 Hansard dataset
# - str_detect() identifies and removes bigrams that contain common stop words
# - The second str_detect() keeps only bigrams made up of two lowercase alphabetic words
...
# Display the cleaned bigrams and associated metadata as a formatted table
df_head %>%
  kable(caption = "Cleaned bigrams from the 1867 Hansard corpus together with their metadata. 
                   The cleaned bigrams reflect preprocessing steps that remove stop words.")
```
...
# Display sample Hansard text containing the phrase "public meetings"
my_reading %>%
  head() %>%
  mutate(text = str_wrap(text, 80)) %>%
  select(text) %>%
  kable(caption = "Sample Hansard text from 1867 containing multiple occurrences
...
```{r, message=FALSE}
# Create 5-grams from the "text" column of the 1867 Hansard dataset
# - unnest_tokens() splits the text into sequences of consecutive words
# - token = "ngrams" specifies that the text should be tokenized into n-grams
# - n = 5 specifies that each n-gram should contain five consecutive words
# - The new column containing the 5-grams is called "ngram"
fivegrams_1867 <- hansard_1867 %>% # Create a new dataset
  unnest_tokens(ngram, text, token = "ngrams", n = 5) # Generate 5-grams from text

# Identify the most frequent 5-grams that contain the word "people"
# - str_detect() keeps 5-grams that contain the word "people"
# - count() counts the frequency of each 5-gram
# - top_n() keeps the 50 most frequent 5-grams
people_n_grams <- fivegrams_1867 %>% # Create a new dataset
  filter(str_detect(ngram, "people")) %>% # Keep 5-grams containing "people"
  count(ngram) %>% # Count the frequency of each 5-gram
  top_n(50) # Keep the top 50 by count
...
Reading further down the list of multi-word phrases might give us more hints about the language with which speakers urged the principle of representation.  Again, the careful analyst would trace the vocabulary words back to their original context, using the original text of the speeches to develop an argument about why a phrase like "the minds of the people" was used.
...
# Display sample Hansard text containing the phrase "minds of the people"
minds %>%
  head() %>%
  mutate(text = str_wrap(text, 80)) %>%
  select(text) %>%
  kable(caption = "Sample Hansard text from 1867 containing multiple 
...
# Add matching debate metadata to the Hansard speech data
hansard_1860 <- left_join(hansard_1860, debate_metadata_1860)
...
```{r}
# Create bigrams from the "text" column of the 1867 Hansard data
# - unnest_tokens() splits the text into sequences of consecutive words
# - token = "ngrams" specifies that the text should be tokenized into n-grams
# - n = 2 specifies that each n-gram should contain two consecutive words
# - The new column containing the bigrams is called "bigram"
bigrams_1867 <- hansard_1867 %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2)

# Clean the bigrams in the 1867 Hansard data
# - str_detect() identifies bigrams that contain common stop words
# - paste() combines the stop words into a single search pattern
# - filter() removes bigrams containing stop words
# - The second str_detect() keeps only bigrams containing two lowercase alphabetic words
clean_bigrams_1867 <- bigrams_1867 %>% 
...
# Filter bigrams using the controlled vocabulary
# - paste() combines the vocabulary terms into a single search pattern
# - str_detect() identifies bigrams containing a vocabulary term
# - filter() keeps only bigrams that contain a vocabulary term
# - select() keeps only the bigram and its associated speech date
...
If we look carefully at the events list so generated, we will recognize only a few of the references as actual events.  Many of the bigrams including our controlled vocabulary are simply descriptions, for example, "disorderly riot."  A few are references to actual events and meetings specified by the name of an event, for instance, the "Bristol Riot" or a "Yorkshire meeting."  Far more frequent are general references to a "public meeting" or "recent meeting."  All of the results are interesting, of course, because as we have seen members of parliament leaned on each other to acknowledge the public sentiment for expanding the vote.  

How did references to meetings vary over time? By this time, it might be clear that searching for a controlled vocabulary, visualization, guided reading, and deliberation about the meaning of the data is an iterative process that we engage over many rounds -- not an automatic process where the analyst looks for a word like "riot" and immediately finds meaningful results.  We can search for our key phrases and visualize them.  

In the following code, we define a new list of bigrams that make up our controlled vocabulary—specific phrases referring to historical events—and then filter the dataset to keep only the rows where the phrases appear. Note that the code below uses a new command for faceted counting--`group_by()`--which we will investigate in greater detail throughout the book. 

For now, it is just important to know that `group_by()` effectively sorts data into buckets based on some category. For example, we could `use group_by()` to organize the data by year—putting all the speeches from the same year into one group—or by speaker, grouping together everything each person said. By grouping, we are able to perform calculations on each category individually. Instead of counting words for an entire data subset, like we did in Chapter 1, we can count words by group. In the following code we use `group_by()` to combine and count each bigram.
...
# Identify the 10 most frequently mentioned historical events
# - filter() keeps bigrams that exactly match a defined historical event
# - group_by() groups occurrences of the same event
# - summarize() counts the total occurrences of each event
# - slice_max() keeps the 10 most frequently mentioned events
# - arrange() sorts the events from most to least frequent
top_events_1867 <- events_1867 %>%
  filter(bigram %in% controlled_vocab_events) %>% # Keep defined events
  group_by(bigram) %>% # Group by event
  summarize(total = n()) %>% # Count occurrences
  slice_max(total, n = 10) %>% # Keep top 10
  arrange(desc(total)) # Sort by frequency

head(top_events_1867)
```

```{r}
# Add speech-date information to the most frequently mentioned events
# - left_join() adds speech dates from the full events dataset
# - mutate() extracts the month from each speech date
# - count() counts event mentions by speech date
# - ungroup() removes any remaining grouping
top_events_1867_w_speech_metadata <- top_events_1867 %>%
  left_join(events_1867, by = "bigram") %>% # Add speech dates
  mutate(month = month(speechdate)) %>% # Extract month
...
In this visualization, it may be clear that we have produced a different sort of visualization than the bar charts we have been creating. `ggplot2` is a vast library for creating many different types of visualizations from the same dataset. Rather than the command for a bar chart -- `geom_col()` -- we have used a command for a dot plot -- `geom_point()`. 
...
What do we make of this timeline? It suggests a swell of public meetings referenced in parliament in the months leading up to August 1867, which received more intense attention than either the Crimean War or the Paris Exhibition.  Individual meetings such as those in Yorkshire or Clontarf received passing acknowledgment rather than deeper debate.  Understanding the results of the visualization deeper requires more cycles of research and reading.  
...
eventslist <- events %>% # Create a new dataset
  ...
  select(event_name, scholar_assigned_date) # Keep name and date
...
We will now count events mentioned in parliamentary speeches, starting with using an `inner_join()`.  Like its cousin, `left_join()`, `inner_join()` works with two data sets to make a new, combined data set.  We use `inner_join()` when we want to analyze shared areas of overlap between two data sets. For our analysis. we want to find the n-grams from `annotated_1830` that are also listed as entities in the events controlled vocabulary. 
...
# - Uses an inner join where the bigram matches the cleaned event name
...
# Display sample two-word phrase events from the 1830 Hansard debates
events_1830 %>%
  head() %>%
  mutate(ngram = str_trunc(ngram, 120)) %>%  
  kable(caption = "Sample two-word phrase events from the 1830 Hansard debates.")
```
...
```{r, message=FALSE}
# Count event mentions by speech date
# - mutate() extracts the year from each speech date
# - group_by() groups mentions by event, year, speech date, and a scholar assigned date
# - summarize() counts the number of mentions in each group
# - ungroup() removes the grouping when no longer needed
counted_events <- events_1830 %>% # Create a new dataset
  mutate(year = year(speechdate)) %>% # Extract year
...
```{r, message=FALSE}
# Summarize total mentions of each event and sort by assigned date
# - group_by() groups mentions by event and assigned date
# - summarize() calculates the total number of mentions for each event
# - ungroup() removes the grouping after the totals are calculated
# - arrange() sorts the events chronologically by their assigned date
cleaned_events <- counted_events %>%
  group_by(ngram, scholar_assigned_date) %>% # Group by event and date
  summarize(total = sum(n)) %>% # Calculate total mentions
  ungroup() %>% # Remove grouping
  arrange(scholar_assigned_date) # Sort chronologically

head(cleaned_events)
```

```{r}
# Divide the historical date range into 30 time periods and identify the top event in each
# - mutate() assigns each event to one of 30 chronological time periods
# - group_by() groups events by time period
# - filter() keeps the most frequently mentioned event in each time period
# - ungroup() removes the grouping
# - left_join() adds the corresponding speech information
indexed_events <- cleaned_events %>%
  mutate(time_period = scholar_assigned_date %/% 
           ((max(scholar_assigned_date) - 
               min(scholar_assigned_date)) / 30)) %>% # Assign time period
  group_by(time_period) %>% # Group by time period
  filter(total == max(total)) %>% # Keep top event
  ungroup() %>% # Remove grouping
  left_join(counted_events, 
            by = c("ngram", "scholar_assigned_date")) # Add speech data

head(indexed_events)
```

```{r}
# Set the end date 8 days after the latest speech date
# - Adding 8 days provides extra space at the edge of the visualization
...
# - ggplot() creates the visualization using the indexed_events dataset
# - x = speechdate places the date of each mention on the x-axis
# - y = scholar_assigned_date places the historical date of each event on the y-axis
# - size = n changes the size of each point based on the number of mentions
# - label = paste0() creates a label with the event name and historical date
# - color = n changes the color of each point based on the number of mentions
ggplot(data = indexed_events, # Use the event data
       aes(x = speechdate, # Set the x-axis
           y = scholar_assigned_date, # Set the y-axis
           size = n, # Set point size
           label = paste0("            ", ngram, " (", scholar_assigned_date, ")"), # Create labels
           color = n)) + # Set point color

# Set the color scale for the bubbles
# - scale_color_viridis() applies a color gradient based on the number of mentions
# - breaks = round rounds the values shown in the legend
# - trans = "log" uses a logarithmic scale to make differences easier to see
# - option = "A" selects the viridis color palette
# - discrete = FALSE treats the number of mentions as a continuous value
# - direction = 1 applies the color gradient from low to high values
scale_color_viridis(breaks = round, # Round legend values
                    trans = "log", # Use log scale
                    option = "A", # Set color palette
                    discrete = FALSE, # Use continuous scale
                    direction = 1) + # Set gradient direction
  
  # Set the appearance and placement of the bubbles
  # - scale_size_continuous() sets the range of bubble sizes
  # - geom_point() displays the data as semi-transparent square points
  # - coord_cartesian() allows labels to extend beyond the plot area
  # - scale_x_date() displays the x-axis in one-year intervals
  scale_size_continuous(range = c(1, 10)) + # Set bubble sizes
  geom_point(alpha = .5, shape = 15) + # Create square bubbles
  coord_cartesian(clip = "off") + # Allow labels outside plot
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") + # Show yearly ticks
  
  # Add one label for each historical event
  # - group_by() groups the data by event
  # - sample_n() selects one mention of each event for labeling
  # - x = left_range places the labels at the right side of the plot
  # - y = scholar_assigned_date aligns each label with its historical date
  # - hjust = 0 left-aligns the label text
  geom_text(data = indexed_events %>% # Use event data
              group_by(ngram) %>% # Group by event
              sample_n(1), # Select one mention
              show.legend = FALSE, # Hide label legend
              aes(color = 1, # Set label color
                  x = left_range, # Position labels
                  y = scholar_assigned_date, # Align by event date
                  hjust = 0, # Left-align labels
                  size = 8)) + # Set label size
  
  # Adjust the plot layout for readability
  # - theme() moves the legend, adds space for labels, and adjusts the x-axis text
  # - guides() removes the shape legend
  theme(legend.position = "bottom", # Move legend below plot
        plot.margin = unit(c(1, 250, 50, 1), unit = "pt"), # Add space for labels
        axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 1), # Rotate year labels
        axis.title.x = element_text(vjust = -0.5)) + # Adjust title spacing
...
Why? we find ourselves asking. That verticality of temporal experience--the long perspective on the deep past--seems to be fairly distinct for 1830-32 of all the years in the decade. Why do so many different, varied periods suddenly become relevant all at  once? Is this pattern a general characteristic of moments of political reform -- that the deep past is trawled for a multitude of examples, for and against?  

We continue the explanation through reading.

```{r}
# Add a new column with the extracted year from the speech date
hansard_1830 <- hansard_1830 %>%
  mutate(year = year(speechdate))

# Find different ways of referring to the fifteenth century in speeches from 1832
# - filter() keeps speeches for just the year 1832
# - regex() searches for several possible forms of "fifteenth century"
# - ignore_case = TRUE matches both uppercase and lowercase versions
c15_mentions <- hansard_1830 %>% # Create a new dataset
  filter(year == 1832, # Filter for 1832
         str_detect(text,
                    regex("fifteenth century|fifteenth-century|15th century|15th-century",
                          ignore_case = TRUE))) %>% # Find references to the fifteenth century
  select(text) # Keep the text column
```

```{r}
# Display the filtered speech texts that mention "fifteenth century"
c15_mentions %>%
  head() %>%
  mutate(text = str_wrap(text, 80)) %>%
  select(text) %>%
...
```{r}
# Filter speeches from 1832 that mention "sixteenth century"
# - filter() keeps speeches for just the year 1832
# - str_detect() keeps speeches containing the phrase "sixteenth century"
# - select() keeps only the text column
c16_mentions <- hansard_1830 %>% # Create a new dataset
  filter(year == 1832, # Filter for 1832
         str_detect(text,
                    regex("sixteenth century|sixteenth-century|16th century|16th-century",
                          ignore_case = TRUE))) %>% # Find references to the sixteenth century
  select(text) # Keep the text column
```

```{r}
# Display sample speech text mentioning "sixteenth century"
c16_mentions %>%
  head() %>%
  mutate(text = str_wrap(text, 80)) %>%
  select(text) %>%
...
```{r}
# Filter speeches from 1832 that mention "seventeenth century"
# - filter() keeps speeches for just the year 1832
# - str_detect() keeps speeches containing different forms of "seventeenth century"
# - select() keeps only the text column
c17_mentions <- hansard_1830 %>% # Create a new dataset
  filter(year == 1832, # Filter for 1832
         str_detect(text,
                    regex("seventeenth century|seventeenth-century|17th century|17th-century",
                          ignore_case = TRUE))) %>% # Find references to the seventeenth century
  select(text) # Keep the text column
```

```{r}
# Display sample speech text mentioning "seventeenth century"
c17_mentions %>%
  head() %>%
  mutate(text = str_wrap(text, 80)) %>%
  select(text) %>%
...
- Load the Dataset: Import the dataset into the R environment using appropriate functions like `data()`.

- Filter or Subset the Dataset: Narrow down the dataset to include only the data relevant to the analysis. For example, if we are analyzing speeches by William Gladstone, we can filter rows where the speaker is Gladstone using functions like `filter()` from the `dplyr` package. Similarly, we could filter for speeches belonging to a specific debate by filtering the data based on a title. Subsetting early in the process reduces the amount of data that will be processed later, saving processing time and computational resources. 

- Clean the Dataset: After subsetting, clean the dataset to prepare it for analysis. Cleaning typically involves handling missing values, removing stop words, and normalizing text (e.g., converting text to lowercase). We can clean data with functions like `mutate()` from `dplyr`, or string processing functions like `str_to_lower()`, which transforms text to lower case, from the `stringr` package.

- Analyze the Dataset: Now that the data has been filtered, apply analytical metrics to identify patterns and answer the research question. This may include counting occurrences, calculating averages or proportions, or using statistical methods to examine trends and relationships in the data.

Although it is possible clean the entire dataset before narrowing it down, doing so can strain our computer's processing power or even exceed its available memory. It is often more desirable to filter the dataset first, then clean the smaller, more manageable portion. Following the order: Load -> Subset -> Clean -> Analyze can optimize data processing, which may be especially useful when analyzing change over decades or centuries of data. 

## Exercises

1) In the code above, we searched for 5-word phrases that reference "the people." Use the code to find two-word phrases in 1830, 1840, 1850, and 1860 that reference "meeting."  Write a paragraph, quoting at least 10 phrases from each decade.  For each of the phrases mentioned more than five times, describe what prejudices, positive or negative, the phrase encapsulates. Use evidence to answer the question: how did parliamentary references to meetings change between 1830 and 1860?  

Tips: Instead of joining every decade together, process the decades separately to avoiding using all the computer's resources. 

2) In the code above, we created two kinds of timelines, one for 1867 using a controlled vocabulary of words such as "meeting" and the other for the 1830s using the list of historical events, `eventlist`. Make two more timelines.  Apply the controlled vocabulary to the 1830s, and search for the items in `eventlist` in the 1860s. Use the code above to graph the results. Keep in mind that one may need to adjust the controlled vocabulary to what is found in the data.  

3) Analyze the results of the timelines.  Use iterative investigations of phrases, as modeled above, to conduct a distant reading of the parliamentary speeches.  Using at least ten examples, compose a one-page essay investigating the question: how did parliamentary speakers use references to contemporary and historical events to make arguments for or against political change?

4) Compare the language of "reform" across decades by finding three-word phrases containing the word "reform." Count the 25 most common phrases in each decade. Then compare the results. Based on the top 25 words, does the language seem to have changed much between these decades? If not, can we find which phrases appear only in one decade and not the other using `anti_join()`?

5. How did Parliament describe "the people" in the 1830s? Generate five-word phrases containing "people" separately for each decade. Count the most frequent phrases. 

6. How did Parliament refer to different forms of political protest in the 1830s and 1860s? Use `str_detect()` and `regex()` to search the original speeches for multiple forms of protest-related words, such as "protest", "protests", "protesting", "agitation", "agitate", "demonstration", and "demonstrations". Compare the results between the two decades. Read at least ten examples from each decade and answer: Did parliamentary descriptions of political protest change between the 1830s and 1860s?

7. Can distant reading help us arrive at questions that close reading can answer? Create one of the chapter's event timelines for a new period. Identify one surprising cluster, increase, absence, or unusual historical reference in the visualization. Then use `str_detect()` to return to the relevant speeches and explain what produced the pattern.

8. Do the most frequent phrases necessarily provide the most historically useful evidence? Generate the top 100 bigrams for one year or period and compare the highest-frequency phrases with less frequent phrases connected to the chapter's controlled vocabulary. Which phrases lead to more useful understanding of Hansard, and why?
